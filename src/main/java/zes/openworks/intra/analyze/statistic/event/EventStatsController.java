...
@Controller
@RequestMapping(value = "/intra/analyze/statistic/event")
public class EventStatsController extends GeneralController {

    @Resource
    EventStatsService service;
    ...
    public void index(HttpServletRequest request, ModelMap model) throws Exception {
        ...
        Map<String, Object> resultMap = service.eventList(searchMap);

        model.addAttribute(GlobalConfig.KEY_SEARCH_MAP, searchMap);
        model.addAttribute(GlobalConfig.KEY_DATA_LIST, resultMap.get("list"));
    }

    @RequestMapping(value = "PD_view.do")
    public void view(HttpServletRequest request, ModelMap model) throws Exception {
        Map<String, Object> searchMap = RequestUtil.getSearchMap(request);
        Map<String, Object> resultMap = service.eventView(searchMap);
        model. addAttribute(GlobalConfig.KEY_DATA_VO, resultMap.get("view"));
    }
}
