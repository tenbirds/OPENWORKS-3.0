...
public class SmsSender implements Sender {
    ...
    public boolean send() {
        ...
        String senderTelNum = props.getProperty("sender.tel");
        if(senderTelNum == null) {
            senderTelNum = senderTel;
        }

        logger.debug("senderName is {}, senderTelNum is {} ", senderName, senderTelNum);
        ...
        try {
            ...
            if(receiptList != null) {
                for(Map<String, String> receipt : receiptList) {
                    ...
                    String mobNo = receipt.get("MOB_NO");

                    logger.debug("useris {}, content is {} ", (userName + ":" + mobNo), content);
                    // 설정된 프로퍼티를 사용하여 전송관련 로직 코딩
                }
            }
        } catch (Exception e) {
            if(logger.isDebugEnabled()) {
                logger.info("{} 전송을 실패하였습니다.", "SMS ", e);
            }
