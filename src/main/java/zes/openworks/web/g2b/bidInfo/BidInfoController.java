 ...
 */
@Controller
@RequestMapping(value = "/{pathLang}/pt/g2b/bidInfo")
public class BidInfoController extends WebController{

    @Autowired BidInfoService bidInfoService;
    ...
    public synchronized String search(HttpServletRequest request, ModelMap model, BidInfoVO vo,@PathVariable String pathLang) throws Exception {
        ...
        if(Validate.isEmpty(vo.getFlag()) || vo.getFlag().startsWith("1001")){
            ...
            vo.getDataMap().put("searchType", vo.getSearchType());
            vo.getDataMap().put("searchKey", vo.getSearchKey());
            vo.getDataMap().put("searchVal", vo.getSearchVal());
            model.addAttribute(GlobalConfig.KEY_PAGER, bidInfoService.bidInfoList(vo));
