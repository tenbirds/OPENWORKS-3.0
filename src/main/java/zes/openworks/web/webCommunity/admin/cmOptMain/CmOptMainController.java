 ...
 * with OMP Inc. 
 */
package zes.openworks.web.webCommunity.admin.cmOptMain;
...
import zes.openworks.web.webCommunity.user.cmUserMain.CmUserMainService;
 ...
 */
@Controller
@RequestMapping(value = "/**/cm/admin/cmOptMain")
public class CmOptMainController extends WebController {
    ...
    public String index(HttpServletRequest request, ModelMap model, CmManageBoardVO vo , CmManageVO cmv){
        ...
        /* {{ 2016.05.04 일반 사용자들의 커뮤티니 관리자 홈 URL접근 차단 로직 추가 */
        CmManageVO dataVo1 = cmManageService.CmOptMainInfo(cmv); // 커뮤니티 개설자 아이디
        ...
        if(userLoginVo != null){
           if(LoginId.equals(AdminId)){
               ...
               vo.setOthbcSetupCdChk("4"); //1001 전체공개 1002 멤버공개 1003 작성자만 체크 벨류
               cmv.setCmmntyMberId(userLoginVo.getUserId());
               cmv.setOthbcSetupCdChk("4");
            }
            else{
                return alertAndBack(model,"접근권한이 없습니다.");
            }
        }/*else{
            vo.setOthbcSetupCdChk("4");
            cmv.setOthbcSetupCdChk("4");
        }*/
