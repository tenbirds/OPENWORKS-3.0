 ...
 * 1.2 or above.
 * <p>
 * Provides common configuration properties and parsing functionality for
 * multipart requests, using a Map of Spring CommonsMultipartFile instances as
 * representation of uploaded files and a String-based parameter Map as
 * representation of uploaded form fields.
 * <p>
 * Subclasses implement concrete resolution strategies for Servlet or Portlet
 * environments: see CommonsMultipartResolver and
 * CommonsPortletMultipartResolver, respectively. This base class is not tied to
 * either of those APIs, factoring out common functionality.
 * 
...
public abstract class CommonsFileUploadSupport {
    ...
    /**
     * Set the maximum allowed size (in bytes) before uploads are written to
     * disk.
     * Uploaded files will still be received past this amount, but they will not
     * be
     ...
     * Default is the servlet container's temporary directory for the web
     * application.
     * 
     ...
     * containing Spring MultipartFile instances and a Map of multipart
     * parameter.
     * 
    ...
    protected MultipartParsingResult parseFileItems(List<FileItem> fileItems, String encoding) {
        ...
        for(FileItem fileItem : fileItems) {
            if(fileItem.isFormField()) {
                ...
                /*
                 * 파일 설명 추가로 인한 데이터 길이 불일치 제거
                 * if(Validate.isEmpty(fileItem.getName())) {
                 * continue;
                 * }
                 */
     ...
     *      protected void cleanupFileItems(Collection<MultipartFile>
     *      multipartFiles) {
     ...
     *      if (logger.isDebugEnabled()) {
     *      logger.debug("Cleaning up multipart file [" + cmf.getName() +
     *      "] with original filename [" +
     *      cmf.getOriginalFilename() + "], stored " +
    ...
    /*-- Added by wisepms --*/
    protected void cleanupFileItems(List<MultipartFile> fileList) {
    ...
    protected static class MultipartParsingResult {
        ...
        /*-- Added by wisepms --*/
        private final List<MultipartFile> multipartList = new ArrayList<MultipartFile>();
        ...
        public MultipartParsingResult(MultiValueMap<String, MultipartFile> mpFiles,
            ...
            /*-- Added by wisepms --*/
            Iterator<String> fileIterator = multipartFiles.keySet().iterator();
            while(fileIterator.hasNext()) {
                String key = fileIterator.next();
                multipartList.addAll(multipartFiles.get(key));
            }
        }
        ...
        /*-- Added by wisepms --*/
        public List<MultipartFile> getMultipartList() {
