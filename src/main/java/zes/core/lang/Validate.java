...
public final class Validate {
    ...
    public static boolean isEqualsDataFormat(final CharSequence propertyValue, String datePattern) {
        ...
        if(patternLowerCase.length() == value.length()) {
            ...
            if(value.matches(valuePattern)) {
                ...
                calendar.add(Calendar.MONTH, mm - 1);
    ...
    /**
     * 개인신상 or (연속문자 or 동일문자) 4회연속 사용 시 false
     *
     * @param propertyValue
     * @return
     */
    public static boolean validPwd(String pwd, String userId, String brthdy, String email, String mbtlnum) {
       ...
       //개인정보 포함 제외시킴
        if( isNotEmpty(userId) && pwd.indexOf(userId) != -1) {
            return false;
        }
        if( isNotEmpty(brthdy) && pwd.indexOf(brthdy) != -1) {
            return false;
        }
        if( isNotEmpty(email) && pwd.indexOf(email) != -1) {
            return false;
        }
        if( isNotEmpty(mbtlnum) && pwd.indexOf(mbtlnum) != -1) {
            return false;
        }
       
        //연속된 숫자 카운터(간단한 문자나 숫자의 연속사용(4자리이상) 금지)
        for (int inx = 0; inx < pwd.length(); inx++) {
     ...
     tmppwd = pwd;
        ...
        for (int inx = 0; inx < tmppwd.length(); inx++) {
            if(!isSpecial(tmppwd.substring(inx, inx + 1)) ){
                String stChar = ".*" + tmppwd.substring(inx, inx + 1) + tmppwd.substring(inx, inx + 1) + tmppwd.substring(inx, inx + 1) + tmppwd.substring(inx, inx + 1) + ".*";
                 if( tmppwd.matches(stChar) ) {
                     return false;
                 }
            }
        }
    ...
    //숫자, 특수문자 외 값 여부
    public static boolean isInteger(String varCk) {
        boolean chk=true;
        String ch= "";
        String charSet = "~`@#$%^&*()|\\+=-_;:\"<>!?/";
        for (int i=0; i<varCk.length(); i++) {
            ...
            if (isDigits(ch)) {
                ...
                for (int j=0; j<charSet.length(); j++) {
                    String comp = charSet.substring(j,j+1);
                    if (ch.equals(comp)) {
                        chk = true;
                        break;
                    }
    ...
    //특수문자 포함여부
    public static boolean isSpecial(String info)  {
        String chars = "~`@#$%^&*()|\\+=-_;:\"<>!?/{}";
        for (int inx = 0; inx < info.length(); inx++) {
    ...
    //숫자 포함여부
    public static boolean isInNumber(String info) {
