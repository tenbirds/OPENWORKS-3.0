...
Ext.onReady(function(){
    ...
    treePanel.on("movenode", function(tree, node, oldParent, newParent, index) {
        if ( typeof(window.jsMenuMove) == "function" )
            eval( jsMenuMove(node, oldParent, newParent, index) );
        else
            jsWarningBox("jsMenuMove(node, oldParent, newParent, index) 함수를 정의하세요.");
    });
    ...
    treePanel.on("load", function(node) {
        if(treePanel.root.hasChildNodes()) {
            ...
            if($("#domainList :selected").val() != "") {
                $("#treeDetailDiv").hide();
                $("#siteName").text($("#domainList :selected").text());
                $("#addMenu").show();
            }
        }
    }, this);
