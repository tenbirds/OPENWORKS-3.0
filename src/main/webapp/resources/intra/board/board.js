...
var onReadyEventFunctions = function() {
    ...
    } else if((OpenWorks.pageType == "INSERT") || (OpenWorks.pageType == "UPDATE") || (OpenWorks.pageType == "REPLY")) {
...
var jsListProcess = function() {
    ...
    $("input[name=seqs]").click(function() {
        if($(this).is(":checked")) {
            ...
            if($("#noticeBbscttAt_" + $(this).val()).val() == 'Y') {
                $(this).parent().parent().addClass("bg_blue");
            } else {
    ...
    if(OpenWorks.boardType == "GALLERY")
        OpenWorks.showSummaryYn = "Y";
    ...
    $("#excelBtn").bind("click", {}, function() {
        $(this).colorbox({
            open : true,
            title : "목록 EXCEL 다운",
            href : "PD_convert.form.do?type=excel&domainCd=" + OpenWorks.domainCd + "&bbsCd=" + OpenWorks.bbsCd,
            width : "650",
            height : "420",
            iframe : true
        });
    ...
    $("#pdfBtn").bind("click", {}, function() {
        $(this).colorbox({
            open : true,
            title : "목록 PDF 다운",
            href : "PD_convert.form.do?type=pdf&domainCd=" + OpenWorks.domainCd + "&bbsCd=" + OpenWorks.bbsCd,
            width : "650",
            height : "420",
            iframe : true
        });
...
var jsSetDay = function(smonth, sday, eday) {
    ...
    if($('#q_endDt').val() != "") {
        ...
        $('#q_startDt').datepicker('setDate',
                new Date(sDay.getFullYear(), sDay.getMonth() - eval(smonth), sDay.getDate() - eval(sday)));
        ...
        var vday = dayVal.substring(6, 8);
        var sDay = new Date(Number(vyear), Number(vmonth) - 1, Number(vday));
        $('#endDtVal').datepicker('setDate',
                new Date(sDay.getFullYear(), sDay.getMonth() + eval(smonth), sDay.getDate() + eval(sday)));
    } else {
        $('#q_startDt').datepicker('setDate',
                new Date(today.getFullYear(), today.getMonth() - eval(smonth), today.getDate() - eval(sday)));
        $('#q_endDt').datepicker('setDate',
                new Date(today.getFullYear(), today.getMonth(), today.getDate() - eval(eday)));
    }
};

// 목록 완전 삭제 버튼
var jsDeleteList = function(bbsCode,domainCode ) {
    ...
    $("#dataForm input[name=bbscttSeq]").val(selectedSeqs);
    jsRequest("dataForm", "ND_board.list.delete.do", "post");
    
};

// 목록 플래그 삭제 버튼
var jsFlagDeleteList = function(domainCode, bbsCode) {
    var selectedSeqs = jsCheckedArray();
    if(selectedSeqs.length == 0) {
        jsWarningBox("삭제 대상 게시물을 1개 이상 선택하세요.");
        return false;
...
// 목록보기에서 이동폼을 호출
var jsTransferList = function(domainCode, bbsCode) {
    var selectedSeqs = jsCheckedArray();
    if(selectedSeqs.length == 0) {
        jsWarningBox("이동할 대상 게시물을 1개 이상 선택하세요.");
        return false;
    }
    var urlStr = "PD_transfer.form.do?domainCd=" + domainCode + "&bbsCd=" + bbsCode;
...
// 수정폼버튼(bbsCd, seq로 구분 refrnSeq = null)
...
// 답글폼버튼(bbsCd, refrnSeq로 구분 seq = null)
...
// 상세보기에서 이동 폼을 호출
var jsTransfer = function(domainCode, bbsCode) {
    var urlStr = "PD_transfer.form.do?domainCd=" + domainCode + "&bbsCd=" + bbsCode + "&seqs=" + $("#dataForm input[name=seq]").val();
    $.fn.colorbox({
        open : true,
        title : "게시글 이동",
        href : urlStr,
...
var SharedOpenWorks = {
    ...
    jsChangeCategoryEvent : function(pageType) {
        if(OpenWorks.useCategory == "Y") {
            ...
            if(pageType == "LIST")
                $("#q_ctgCd").bind("change", {}, function() {
                    jsListReq(1);
                });
                $("#q_aditCtgCd").bind("change", {}, function() {
                    jsListReq(1);
                });
                $("#q_othbcAt").bind("change", {}, function() {
                    jsListReq(1);
                });
                $("#q_estnColumn3").bind("change", {}, function() {
                    jsListReq(1);
                });
                
                if(OpenWorks.bbsCd == 1002 ){
                    jsaditCtgCd();
                }
    ...
    jsFileDialogEvent : function() {
        $("a.fileDown").cluetip({
            ...
            clickThrough : true,
            waitImage : true,
            arrows : true, // if true, displays arrow on appropriate side of
                            // clueTip. more info below [8]
            dropShadow : true, // set to false if you don't want the
                                // drop-shadow effect on the clueTip
            dropShadowSteps : 6,
            closePosition : 'top', // location of close text for sticky
                                    // cluetips; can be 'top' or 'bottom' or
                                    // 'title'
            closeText : '닫기',
            cluetipClass : 'jtip',
            activation : 'click'
...
//                    alert($("#dataForm input[name=multiFiles]").length);// $("input:file").length
                },
                        ...
                        $(form).ajaxSubmit(
                                {
                                    ...
                                    beforeSubmit : function(arr, $form, options) {
                                        if((timeWait = jsCheckCookieTime("scoreCookie:" + OpenWorks.bbsCd + ":"
                                                + OpenWorks.seq)) > 0) {
                                    ...
                                    success : function(data) {
                                        if(data.value != 0) {
                                            ...
                                            jsSetCookieExpTime("scoreCookie:" + OpenWorks.bbsCd + ":" + OpenWorks.seq,
                                                    OpenWorks.readCookieHour);
                                        } else {
...
var refreshPage = function() {
    if(OpenWorks.pageType == "LIST")
        jsListReq(1);
    else if(OpenWorks.pageType == "VIEW")
        jsViewReq(1);
};
...
var jsSearch = function() {
    ...
    //if($("#q_domainCds").val() != null && $("#q_domainCds").val() != ""){
        var domainCds = "";
        $("input[name*=q_domainCd]").each(function(){
            
            if($(this).attr("checked")=="checked"){
                domainCds += $(this).val()+"";
            }
        });
        $("#q_domainCds").val(domainCds);
    //}
...
var jsSearchReset = function() {
    ...
    $("#dataForm input[name=q_othbcAt]").val("");
    $("#dataForm input[name=q_domainCd]").val("");
    $("#dataForm input[name=q_domainCdAll]").val("");
    $("#dataForm input[name=q_estnColumn3]").val("");
    $("#dataForm input[name=q_estnColumn5]").val("");
    $("#bizProgression").val("");
    alert("1111")
    $("#dataForm input[name=q_searchKeyType]").val("");
//        document.dataForm.elements["q_ctgCd"].value = "";
//        document.dataForm.elements["q_aditCtgCd"].value = "";
//        document.dataForm.elements["q_othbcAt"].value = "";
//        document.dataForm.elements["q_domainCd"].value = "";
//        document.dataForm.elements["q_domainCdAll"].value = "";
//        document.dataForm.elements["q_estnColumn3"].value = "";
//        document.dataForm.elements["q_estnColumn5"].value = "";
//        document.dataForm.elements["bizProgression"].value = "";
...
// 내용보기 클릭시
var jsView = function(domainCd, bbsCd, seq, regPwd, openYn) {
    if(seq == "") {
     ...
     * pwd = window.prompt("게시물 비밀번호를 입력하세요", ""); if(pwd == null) return;
     * }else{ pwd = ""; } $.post("BD_board.view.do", { bbsCd: $("#dataForm
     * input[name=bbsCd]").val(), authType : "VIEW", seq : seq, regPwd : pwd },
     * function(result){ if(result.value){ $("#dataForm
     * input[name=bbsCd]").val(bbsCd); $("#dataForm input[name=seq]").val(seq);
     * $("#dataForm input[name=regPwd]").val(pwd); jsRequest("dataForm",
     * "BD_board.view.do", "get"); }else{ jsWarningBox('내용보기 권한이 없습니다.'); return
     * false; } }, "json");
     */
    $("#dataForm input[name=domainCd]").val(domainCd);
    $("#dataForm input[name=bbsCd]").val(bbsCd);
    $("#dataForm input[name=bbscttSeq]").val(seq);
    jsRequest("dataForm", "BD_board.view.do", "get");
};
...
var jsOverDateChk = function(cday, limit) {
    ...
    dday = Math.floor(day);
    if(dday <= Number(limit))
        return true;
    return false;
};

// 게시판 글 사용 여부 설정
var jsYnAction = function(el, domainCd, bbsCd, seq) {
    var element = $(el).children();
    var bannedWordYn = element.attr("yn") == "Y" ? "N" : "Y";
...
// 등록 버튼폼(bbsCd로 구분 refrnSeq=null, seq=null)
...
// 도메인으로 재조회하기
var jsDomainSel = function(domainCd) {
    $("#dataForm input[name=domainCd]").val(domainCd);
    jsRequest("dataForm", "BD_board.list.do", "get");
};

// 2Depth 부류코드 조회하기
var jsaditCtgCd = function() {
    var ctgCd = OpenWorks.ctgCd;
    var aditCtgCd = OpenWorks.aditCtgCd;
        ...
        $.ajax({
            ...
            success: function(datalist) {
                ...
                if(datalist.length > 0) {
                    $("select[id='q_aditCtgCd']").empty();
                    selectString += "<option value='''>- 구분선택 2 -</option>";
                    for(var i = 0; i < datalist.length; i++) {
                     selectString += "<option value='"+datalist[i].aditCtgCd+"'>"+datalist[i].aditCtgNm+"</option>";
                    }
                    $("select[id='q_aditCtgCd']").append(selectString);
                    
                    $("select[id='q_aditCtgCd'] option").each(function(){
                       if($(this).val() == OpenWorks.aditCtgCd){
