...
$(document).ready(function(){
	...
	********************************************/
	$('.btn_func').click(function(){ 
		...
		if( terms.is(':visible') ){ 
			terms.hide(); 
			$(this).removeClass('on').text('Abrir'); /* 2015-01-06 수정 */
		} else { 
			terms.show();
			$(this).addClass('on').text('Cerrar'); /* 2015-01-06 수정 */
		}
	});
...
$.fn.gnbAct  = function () {
 	...
 	$('.gnbNavi li > a').bind('mouseenter focus', function() {
 	    ...
 	    if(thisId == "premium" || thisId == "solution" || thisId == "cnsl"){ 	        
 	        ...
 	        $.ajax({
 	            url      : "/spanish/common/commonduty/ND_ctgryView.do",
 	            type     : "POST",
 	            ...
 	            success  : function(response){
 	                ...
 	                if(ctgryTotal > 0){
 	                    for(var i = 0; i < ctgryTotal; i++){
 	  	                    $('<li><a href="/spanish/pt/store/' + thisId + '/BD_index.do?ctgryCode=' + response.ctgryList[i].id + '" id="' + response.ctgryList[i].id + '" onmouseover="javascript:get2dCtgry(this);" onfocus="javascript:get2dCtgry(this);">' + response.ctgryList[i].text + '</a><ul id="depth2_' + (i+1) + '" class="depth2"></ul><ul id="depth2_more_' + (i+1) + '" class="depth2"></ul></li>').appendTo(".depth1");
 	                    }
 	                }
 	                //최초 2Depth
                    if(ctgry2Total > 0){
                        ...
                        for(var j = 0; j < ctgry2Total; j++){
                            if(j < 7){
                                $('<li><a href="/spanish/pt/store/' + thisId + '/BD_index.do?ctgryCode=' + response.ctgry2List[j].id + '" id="' + response.ctgry2List[j].id + '">' + response.ctgry2List[j].text + '</a></li>').appendTo("#"+thisId+"_depth1 #depth2_1");
                            }else{
                                $('<li><a href="/spanish/pt/store/' + thisId + '/BD_index.do?ctgryCode=' + response.ctgry2List[j].id + '" id="' + response.ctgry2List[j].id + '">' + response.ctgry2List[j].text + '</a></li>').appendTo("#"+thisId+"_depth1 #depth2_more_1");
                            }
                        }
                    }
                    //이벤트
 ...
 function get2dCtgry(el){
     ...
     $.ajax({
         url      : "/spanish/common/commonduty/ND_ctgry2dView.do",
         type     : "POST",
         ...
         success  : function(response){
            ...
            if(ctgryTotal > 0){
                ...
                for(var a = 0; a < ctgryTotal; a++){
                    if(a < 7){
                        $('<li><a href="/spanish/pt/store/' + thisId + '/BD_index.do?ctgryCode=' + response.ctgryList[a].id + '" id="' + response.ctgryList[a].id + '">' + response.ctgryList[a].text + '</a></li>').appendTo("#"+thisId+"_depth1 #depth2_1");
                    }else{
                        $('<li><a href="/spanish/pt/store/' + thisId + '/BD_index.do?ctgryCode=' + response.ctgryList[a].id + '" id="' + response.ctgryList[a].id + '">' + response.ctgryList[a].text + '</a></li>').appendTo("#"+thisId+"_depth1 #depth2_more_1");
                    }
                }
            }
         }
 ...
 function moveEvent(sn){
     location.href="/spanish/pt/webevent/BD_eventView.do?flag=N&eventSn="+sn;
 }
 ...
 function fn_mainSearch(){
     ...
     if(serachKeyword.value == ""){
         ...
         location.href="/spanish/pt/search/BD_index.do?searchKeyword="+encodeURIComponent(serachKeyword.value);
     }
 }
