...
<head>
	...
	<title>소프트웨어스토어 씨앗 - <c:choose><c:when test="${not empty cmsVo.title}"><c:out value="${cmsVo.title}" /> : <c:out value="${cmsVo.menuPath}" escapeXml="true"/></c:when><c:otherwise><c:out value="${cmsVo.menuNm}" /> : <c:out value="${cmsVo.menuPath}" escapeXml="true"/></c:otherwise></c:choose></title>
	<op:jsTag type="openworks" items="mobile-core"/>
	...
	<link rel="stylesheet" href="/cloud_ver3/html_m/css/sub.css">
					...
					<form class="sech_form" onsubmit="return false">
						<fieldset>
							...
							<div class="form" name="searchs" id="searchs"">
								<c:if test="${empty totalKeyword}">
							        <span onkeydown="javascript:if(event.keyCode == 13){ fn_mobMainSearch(); }">
							        	<input type="search" placeholder="클라우드 서비스 검색" title="검색하실 단어 입력" id="totalKeyword" name="totalKeyword" value="" />
							        </span>
						        </c:if>
						        <c:if test="${not empty totalKeyword}">
						        	<span onkeydown="javascript:if(event.keyCode == 13){ fn_mobMainSearch(); }">
						        		<input type="search" placeholder="클라우드 서비스 검색" title="검색하실 단어 입력" id="totalKeyword" name="totalKeyword" value="${totalKeyword}" />
						        	</span>
						        </c:if>
...
<!-- end : nav -->
	...
	<%-- 	<!-- header -->
					...
					<form class="sech_form" onsubmit="return false">
						<fieldset>
							...
							<div class="form"  name="searchs" id="searchs"">
								...
								<input type="button" onclick="javascript:fn_mobMainSearch();" alt="search" class="btn" title="검색" onkeydown="javascript: if (event.keyCode == 13) {enter();}">
							</div>
						</fieldset>
					</form>
					<!-- //search -->
		...
		<!-- container -->
		<div id="container" <c:if test="${cmsVo.topMenuCode == 'mintroduction'}">class="introduce"</c:if>>
