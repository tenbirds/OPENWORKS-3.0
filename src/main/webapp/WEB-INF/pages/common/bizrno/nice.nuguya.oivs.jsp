...
///	#############################################################################
///	#####
///	#####	한국신용정보주식회사			기업 기업실명확인 서비스 처리 스크립트
///	#####
///	#####	=====================================================================
///	#####
///	#####	Descriptions
...
public class OivsObject 
{
	...
	public String coTypeCd		= "";
	public String coStatCd		= "";
	...
	private String[] resolveReturnValue( String returnValue )
	{
		...
		{
			this.message = "기업실명확인 서비스에서 반환한 결과를 처리할 수 없습니다. [nice.nuguya.oivs : resolveReturnValue] \\n\\n고객상담센터(☎ 1588-2486) 로 연락하십시오";
			return aryValues;
		}
	...
	public boolean desClientData()
	{
		...
		try
		{
			...
			this.bizNo 		= aryValues[0];
			this.bizNm 		= URLDecoder.decode( aryValues[1], CHARSET_SERVER );
			this.inqRsn 	= aryValues[2];
			this.infoFlag	= aryValues[3];

			return true;
		}
		catch( Exception ex )
		{
			this.message = "기업실명확인 서비스에서 오류가 발생하였습니다. [nice.nuguya.oivs.inc : desClientData(callSerivce exception)] \\n\\n고객상담센터(☎ 1588-2486) 로 연락하십시오" +
				"\\n\\n[오류정보]\\n" + ex.getMessage();
			return false;
		}
	}
	...
	public boolean callService()
	{
		...
		try
		{
			...
			{
				this.message = "기업실명확인 서비스와의 보안 통신에 필요한 정보가 부족합니다. [nice.nuguya.oivs.inc : callService(server getServiceKey)] \\n\\n고객상담센터(☎ 1588-2486) 로 연락하십시오";
				return false;
			}
			else if ( ! aryValues[3].equals( "" ) )
			{
				this.message = "기업실명확인 서비스와의 보안 통신중에 오류가 발생하였습니다. [nice.nuguya.oivs.inc : callService(server getServiceKey)] \\n\\n고객상담센터(☎ 1588-2486) 로 연락하십시오" +
						" \\n\\n[오류정보]\\n\\n" + aryValues[3];
				return false;
			}
			...
			if ( this.retCd.equals( "0" ) && this.retDtlCd.equals( "0" ) )
			{
				...
				bytEncData = this.TriDES_EncryptString( this.skData.getBytes( CHARSET_DEFAULT ), this.niceId + DELIMETER_NORMAL + this.bizNo + DELIMETER_NORMAL + 
								URLEncoder.encode( this.bizNm, CHARSET_SERVER ) + DELIMETER_NORMAL + this.inqRsn + DELIMETER_NORMAL + this.infoFlag );
				strCDESInfo = this.encode( this.addBytes( ( this.identifier + DELIMETER_NORMAL + SVC_TYPE_CORPERATE + DELIMETER_NORMAL ).getBytes(), this.encode( bytEncData ).getBytes() ) );

				//	서비스를 호출한다.
				this.callRealName.setOperationName( new QName( URL_QNAME, "checkCoRealName" ) );
				...
				if ( aryValues == null || aryValues.length < 5 )
				{
					this.message = "기업실명확인 서비스의 결과 정보가 부족합니다. [nice.nuguya.oivs.inc : callService(server checkRealName)] \\n\\n고객상담센터(☎ 1588-2486) 로 연락하십시오";
					return false;
				}
				else if ( ! aryValues[3].equals( "" ) )
				{
					this.message = "기업실명확인 서비스 진행 중에 오류가 발생하였습니다. [nice.nuguya.oivs.inc : callService(server checkRealName)] \\n\\n고객상담센터(☎ 1588-2486) 로 연락하십시오" +
							" \\n\\n[오류정보]\\n\\n" + aryValues[3];
					return false;
				}

				if ( ! aryValues[0].equals( this.getHashedString( this.bizNo ) ) )
				{
					this.message = "기업실명확인 서비스의 결과가 변형되어 정확한 정보가 아닙니다. [nice.nuguya.oivs.inc : callService(server checkRealName.. hash mismatch)] \\n\\n고객상담센터(☎ 1588-2486) 로 연락하십시오";
					return false;
				}
				...
				aryValues		= aryValues[4].split( DELIMETER_DATA );
				this.niceNm 	= URLDecoder.decode( aryValues[0], CHARSET_SERVER );
				this.coTypeCd 	= aryValues[1].trim();
				this.coStatCd 	= aryValues[2].trim();

				return true;
			}
			else
			{
				this.message = "기업실명확인 서비스와의 보안 통신을 계속 진행할 수 없습니다. [nice.nuguya.oivs : callService(server checkRealName)] \\n\\n고객상담센터(☎ 1588-2486) 로 연락하십시오" +
		...
		{
			this.message = "기업실명확인 서비스에서 오류가 발생하였습니다. [nice.nuguya.oivs.inc : callService(callSerivce exception)] \\n\\n고객상담센터(☎ 1588-2486) 로 연락하십시오" +
				"\\n\\n[오류정보]\\n" + ex.getMessage();
			return false;
		}
	}
