...
<head>
    ...
    (function ($) {
        if('${param.pwdYn}' != 'Y'){
            self.location.href = "/intra/user/BD_pwdForm.do?title1="+ "회원정보변경이력관리"+"&title2="+"회원정보변경 이력"+"&menuType="+"L"+"&returnUrl="+"/intra/user/coLog/BD_userLogListU.do";
        }
    })($);
	...
	<div id="contents">
		<div class="tx_r">
			<button type="button" class="s_blue mar_l5 mar_b5" onclick="pwdConfirm(
																					...
																					, ''
																					, ''
																					, '∬q_searchKey=${param.q_searchKey}∬q_searchVal=${param.q_searchVal}∬q_stDate=${param.q_stDate}∬q_enDate=${param.q_enDate}∬logMenuTy=U'
																					, '리스트'
																					, '' 
																					);">엑셀출력</button>
		</div>
		...
		<div>
			<fieldset>
				...
				<select id="q_searchKey" name="q_searchKey" class="over">
					...
					<option <c:if test="${param.q_searchKey == 1004}">selected="selected"</c:if> value="1004">변경자이름</option>
					<option <c:if test="${param.q_searchKey == 1005}">selected="selected"</c:if> value="1005">변경자ID</option>
				</select>
		...
		<op:pagerParam title="회원정보변경이력 목록" />
			...
			<tbody id="odd-color">
				...
				<c:forEach items="${pager.list}" var="_bean" varStatus="status">			
					<tr>
						...
						<td class="tx_l">${_bean.mngrNm}<br/>(${_bean.mngrId})</td>
						<td class="tx_l">${_bean.changeCn}</td>
						<td class="tx_l lr_none">
							<div class="po_rel">
								<a href="#" onclick="reasonLayer('${index-status.index}'); return false;">${fn:substring(_bean.accessResn,0,13)}<c:if test="${fn:length(_bean.accessResn) > 13}">...</c:if></a>
								<!-- 변경사유 레이어 -->
