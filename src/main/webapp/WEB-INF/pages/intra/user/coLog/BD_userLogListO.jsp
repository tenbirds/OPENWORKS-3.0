...
<script type="text/javascript">
    (function ($) {
        if('${param.pwdYn}' != 'Y'){
            self.location.href = "/intra/user/BD_pwdForm.do?title1="+ "회원정보출력이력관리"+"&title2="+"회원정보출력 이력"+"&menuType="+"L"+"&returnUrl="+"/intra/user/coLog/BD_userLogListO.do";
        }
    })($);
...
<body>
    ...
    <div id="contents">
    	<div class="tx_r">
		    <button type="button" class="s_blue mar_l5 mar_b5" onclick="pwdConfirm(
                                                                                   this
                                                                                 , '회원정보조회이력 리스트 엑셀 출력'
                                                                                 , 'O'
                                                                                 ...
                                                                                 , ''
                                                                                 , ''
                                                                                 , '∬q_searchKey=${param.q_searchKey}∬q_searchKey1=${param.q_searchKey1}∬q_stDate=${param.q_stDate}∬q_enDate=${param.q_enDate}∬q_searchVal=${param.q_searchVal}∬menuType=O'
                                                                                 , '리스트'
                                                                                 );">엑셀출력</button>
		</div>
	    <!-- 검색 -->
	        ...
	        <div>
	            <fieldset>
	                ...
	                <select id="q_searchKey" name="q_searchKey" class="over">
	                    <option <c:if test="${param.q_searchKey == ''}">selected="selected"</c:if> value="">메뉴명</option>
	                    <option <c:if test="${param.q_searchKey == 'ALL'}">selected="selected"</c:if> value="ALL">전체</option>
	                    <c:forEach items="${menuNmList}" var="_bean">
	                    	<option <c:if test="${param.q_searchKey == _bean.logMenuNm}">selected="selected"</c:if> >${_bean.logMenuNm}</option>
	                    </c:forEach>
	                </select>
	                <select id="q_searchKey1" name="q_searchKey1" class="over">
	                    <option <c:if test="${param.q_searchKey1 == ''}">selected="selected"</c:if> value="">선택</option>
	                    <option <c:if test="${param.q_searchKey1 == 1001}">selected="selected"</c:if> value="1001">전체</option>
	                    <option <c:if test="${param.q_searchKey1 == 1002}">selected="selected"</c:if> value="1002">출력자</option>
	                    <option <c:if test="${param.q_searchKey1 == 1003}">selected="selected"</c:if> value="1003">출력자ID</option>
	                </select>
	                <input class="w215" type="text" value="${param.q_searchVal}" id="q_searchVal" name="q_searchVal" title="검색어를 입력하세요."/>
	                <button type="button" id="wordSerch" onclick="jsSearchAction();" class="gray mar_l5 mar_b5">검색</button>
	            </fieldset>
	        </div>
	        <!-- //검색 -->
	        <br/>
        ...
        <op:pagerParam title="회원정보출력이력 목록" />
            ...
            <tbody id="odd-color">
                ...
                <c:forEach items="${pager.list}" var="_bean" varStatus="status">
                    <tr>
                        ...
                        <td class="tx_l">${_bean.outptCn}</td>
                        <td>${_bean.accessDt}</td>
                        <td class="tx_l">${_bean.mngrNm}</td>
                        <td class="tx_l">${_bean.mngrId}</td>
                        <td class="tx_l lr_none">
                            <div class="po_rel">
								<a href="#" onclick="reasonLayer('${index-status.index}'); return false;">${fn:substring(_bean.accessResn,0,20)}<c:if test="${fn:length(_bean.accessResn) > 20}">...</c:if></a>
                                <!-- 변경사유 레이어 -->
