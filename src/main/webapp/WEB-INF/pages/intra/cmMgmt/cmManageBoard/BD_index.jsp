...
// 	        });
	   ...
	   var jrMove = function(movepage){
        ...
        var jsPopupView = function(el, cmmntyId, langCode, userId) {
            $(el).colorbox({
               	title : "커뮤니티 승인 정보",         
     			href      : "/intra/cmMgmt/cmManage/PD_openInfoView.do?cmmntyId="+cmmntyId+"&langCode="+langCode+"&userId="+userId,
     			width : "1200px",
     	        height : "600px",
               iframe : true
           }); 
        ...
        // 커뮤니티  게시판 팝업 view  <c:out value='${indata.cmmntyId}'/>','<c:out value='${indata.langCode}'/>', '<c:out value='${indata.cmmntyMenuCode}'/>
        var jsBoardPopupView = function(el, cmmntyId, langCode, cmmntyMenuCode, cmmntyBbsTyCd) {
               $(el).colorbox({
                  	title : "커뮤니티 게시판 정보",         
        			href      : "PD_cmManageBBSList.do?cmmntyId="+cmmntyId+"&langCode="+langCode+"&cmmntyMenuCode="+cmmntyMenuCode+"&cmmntyBbsTyCd="+cmmntyBbsTyCd,
        			width : "1200px",
        	        height : "800px",
                  iframe : true
              }); 
			                ...
			                <tbody>
	                        	...
	                        	<tr>
	                        		...
	                        		<td >
				                        ...
				                        <input class="w215" type="text" value="${param.q_searchKeyWord}" id="q_searchKeyWord" name="q_searchKeyWord" title="검색어를 입력하세요."/>
...
<%--                             <td><input type="checkbox" id="ckMultiBox_<c:out value='${status.count}'/>"  name="ckMultiBox"   value="<c:out value='${data.cmmntyId}'/>_<c:out value='${data.langCode}'/>_<c:out value='${data.userId}'/>"/></td> --%>
                            ...
                            <td>
                                <a href="#none" id="popView_<c:out value='${pager.indexNo - status.index}'/>" onclick="jsPopupView(this,'<c:out value='${data.cmmntyId}'/>','<c:out value='${data.langCode}'/>', '<c:out value='${data.userId}'/>');"><c:out value='${data.cmmntyNm}'/></a>
                            </td>
                           	<td> 
                           	(<c:out value='${data.cmmntyMenuCount}'/>)<br/>
                           	<c:forEach items="${data.cmMberMenuCollection}" var="indata" varStatus="status">
                            	<a href="#none" id="popWindow_<c:out value='${status.count}'/>" onclick="jsBoardPopupView(this,'<c:out value='${indata.cmmntyId}'/>','<c:out value='${indata.langCode}'/>', '<c:out value='${indata.cmmntyMenuCode}'/>','<c:out value='${indata.cmmntyBbsTyCd }'/>');"><c:out value='${indata.cmmntyMenuNm}'/></a><br/>
                            </c:forEach></td>
