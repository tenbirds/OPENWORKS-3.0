						...
						<c:when test="${arrange.columnId eq 'RECOMEND_CNT'}">
							...
							<c:if test="${boardConfVo.recomendYn eq 'Y'}">
								<tr>
									<th><label for="${arrange.beanNm}">${arrange.columnNm}</label></th>
									<td>
										<input type="button" id="recomBtn" class="s_blue vm" value="추천 ${dataVo.recomendCnt}" onclick="jsClickRecommAction(this, ${dataVo.bbsCd}, '${dataVo.bbscttSeq}', ${boardConfVo.readCookieHour});" />
						...
						<c:when test="${arrange.columnId eq 'ACCUSE_CNT'}">
							...
							<c:if test="${boardConfVo.accuseYn eq 'Y'}">
								<tr>
									<th><label for="${arrange.beanNm}">${arrange.columnNm}</label></th>
									<td>
										<input type="button" id="accuseBtn" class="s_blue vm" value="신고 ${dataVo.accuseCnt}" onclick="jsClickAccuseAction(this, ${dataVo.bbsCd}, '${dataVo.bbscttSeq}', ${boardConfVo.readCookieHour});" />
	...
	<div class="mar_t10">
		<div class="float_l mar_b10">
			...
			<c:if test="${boardConfVo.listViewCd eq 1001}">
				<button type="button" class="w_blue mar_r5" onclick="jsView('${dataVo.prevVO.bbsCd}', '${dataVo.prevVO.bbscttSeq}', '${dataVo.prevVO.registPassword}', '${dataVo.prevVO.othbcAt}'); return false;">이전</button>
				<button type="button" class="w_blue mar_r5" onclick="jsView('${dataVo.nextVO.bbsCd}', '${dataVo.nextVO.bbscttSeq}', '${dataVo.nextVO.registPassword}', '${dataVo.nextVO.othbcAt}'); return false;">다음</button>
			</c:if>
		</div>
		<div class="float_r mar_b10">
			...
			<c:if test="${(MenuAssignType eq 'A' && dataVo.regId eq __msk.mngrId) or MenuAssignType eq 'G'}">
				...
				<c:if test="${empty dataVo.mngrDelAt or dataVo.mngrDelAt eq 'N'}">
