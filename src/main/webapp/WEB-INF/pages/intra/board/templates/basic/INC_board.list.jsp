...
<table class="boardList" cellspacing="0" border="0" summary="기본게시판 목록으로 순번,분류,제목,작성자,작성일 등의 조회 정보를 제공합니다.">
	...
	<colgroup>
		...
		<c:forEach items="${boardConfVo.listArrange}" var="arrange" varStatus="status">
			...
			<c:if test="${arrange.columnId ne 'BBSCTT_SJ'}">
			<c:if test="${arrange.columnId ne 'GOODS_NM'}"><col width="10%" /></c:if>
			<c:if test="${arrange.columnId eq 'GOODS_NM'}"><col width="*" /></c:if>
			</c:if>
		</c:forEach>
		
		<!-- 다운로드수 -->
		<c:if test="${boardConfVo.dwldYn eq 'Y' }">
		<c:set var="thCnt" value="${thCnt + 1}" />
		<col width="6%" />
		</c:if>
		
		<c:if test="${boardConfVo.bbsCd eq 1001 }"> <!--공지사항사용 -->
		<c:set var="thCnt" value="${thCnt + 1}" />
        <col width="15%" />
        </c:if>
	...
	<thead>
		<tr>
			...
			<!-- 다운로드 수 -->
			<c:if test="${boardConfVo.dwldYn eq 'Y' }">
			<th id="th-DOWNCNT"><span class="sort cs_pointer" id="DOWNCNT">다운로드수</span></th>
			</c:if>
			
			<!-- 공지사항 사용 -->
			<c:if test="${boardConfVo.bbsCd eq 1001 }">
			<th id="th-BOARD_URL"><span class="sort cs_pointer" id="BOARD_URL">게시판 번호(코드)</span></th>
			</c:if>
		</tr>
	</thead>
	<tbody id="odd-color">
			...
			<c:forEach items="${noticeList}" var="noticeVo" varStatus="status">
                ...
                <td>
                  ...
                  <img src="<c:url value="/resources/openworks/theme/default/images/contents/no_img.gif" />" alt="손상된 이미지"/>
					...
					<c:if test="${boardConfVo.dwldYn eq 'Y' }">
					<td>${noticeVo.downCnt}</td>
					</c:if>
					
					<c:if test="${boardConfVo.bbsCd eq 1001 }">
                     <td>
                        <c:if test="${noticeVo.ctgCd eq 1003}">${noticeVo.bbscttSeq} </c:if>
                     </td>
                 </c:if>
				</tr>
			</c:forEach>
		...
		<c:forEach items="${pager.list}" var="baseVo" varStatus="status">
			<tr>
				<!-- 삭제/이동 등을 위한 체크박스 (목록 필수) -->
				<c:if test="${pageType eq 'LIST'}">
					<td><input type="checkbox" value="${baseVo.bbscttSeq}" id="bbscttSeqs" name="bbscttSeqs" class="checkbox" /></td>
				</c:if>
				...
				<c:forEach items="${boardConfVo.listArrange}" var="arrange" varStatus="arrStatus">
					<c:choose>
						<c:when test="${arrange.columnId eq 'BBSCTT_SJ'}">
							<td class="tx_l">
								<a href="#" rel="#titleSummary${status.count}" class="contentTip" onclick="jsView('${baseVo.domainCd}','${baseVo.bbsCd}', '${baseVo.bbscttSeq}', '${baseVo.registPassword}', '${baseVo.othbcAt}'); return false;">${baseVo.bbscttSj}<c:if test="${baseVo.bbscttSjLength > (boardConfVo.cutTitleNum * 2)}">...</c:if></a>
              ...
              <c:choose>
						...
						<c:if test="${boardConfVo.bbsCd eq 1031 }">
							<td style="text-align: left">
								<c:if test="${boardConfVo.bbsCd eq 1031 }">
									...
									<c:set var="goodsCdArry" value="${fn:split(baseVo.goodsCode, ',') }"/>
										...
										<c:if test="${i.index eq s.index }">
											<li>
												...
												<a href="javascript:makeTab('전체상품목록관리', '/intra/store/goods/BD_goodsDetail.do?goodsCode=${goodsCode}', '241', 'UPDATE');">${goodsName}</a>
				 ...
				 <c:if test="${boardConfVo.dwldYn eq 'Y' }">
				 <td>${baseVo.downCnt}</td>
				 </c:if>
				 
				 <c:if test="${boardConfVo.bbsCd eq 1001 }">
					 <td>
					    <c:if test="${baseVo.ctgCd eq 1003}">${baseVo.bbscttSeq}</c:if>
					 </td>
				 </c:if>
			</tr>
		</c:forEach>
