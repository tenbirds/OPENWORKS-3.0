...
<table class="boardList" cellspacing="0" border="0" summary="기본게시판 목록으로 순번,분류,제목,작성자,작성일 등의 조회 정보를 제공합니다.">
	...
	<thead>
		<tr>
			...
			<!-- 상태(필수) -->
			<th id="th-REPLY_DT" class="lr_none"><span class="sort cs_pointer" id="REPLY_DT">답변일</span></th>
			<!-- 상태(필수) -->
			<th id="th-ESTN_COLUMN3" class="lr_none"><span class="sort cs_pointer" id="ESTN_COLUMN3">상태</span></th>
		</tr>
	</thead>
	<tbody id="odd-color">
		<!-- 공지 목록  -->
		...
		<c:forEach items="${pager.list}" var="baseVo" varStatus="status">
			<tr>
				...
				<c:forEach items="${boardConfVo.listArrange}" var="arrange" varStatus="arrStatus">
					<c:choose>
						...
						<c:when test="${arrange.columnId eq 'ESTN_COLUMN3'}">
							<td>
							<c:if test="${baseVo.estnColumn3 eq 'Y'}" >공개</c:if>
							<c:if test="${baseVo.estnColumn3 eq 'N'}" >비공개</c:if>
							</td>
						</c:when>
						<c:otherwise>
				...
				<!-- 답변일(필수) -->
				<td >${baseVo.replyDt}</td>
				<!-- 상태(필수) -->
				<td class="lr_none">
					<c:choose>
						<c:when test="${baseVo.estnColumn3 eq 'N'}">답변미등록</c:when>
						<c:when test="${baseVo.estnColumn3 eq 'Y'}">답변완료</c:when>
