...
<table class="boardList" cellspacing="0" border="0" summary="기본게시판 목록으로 순번,분류,제목,작성자,작성일 등의 조회 정보를 제공합니다.">
	...
	<colgroup>
		...
		<c:forEach items="${boardConfVo.listArrange}" var="arrange" varStatus="status">
			<c:set var="thCnt" value="${thCnt + 1}" />
			<c:choose>
				<c:when test="${arrange.columnId eq 'TITLE'}"><col width="*" /></c:when>
					...
					<td>
						<c:choose>
							<c:when test="${noticeVo.fileCnt > 0}">
								<c:set var="imageFileCnt" value="0" />
								<c:forEach items="${noticeVo.fileList}" var="fileBean" varStatus="status">
									<c:if test="${fn:startsWith(fileBean.fileType, 'image')}">
										<c:set var="imageFileCnt" value="${imageFileCnt + 1}" />
									</c:if>
									<c:if test="${imageFileCnt eq 1}">
										<img src="<c:url value="${fileBean.fileUrl}" />" class="w_100p" title="${fileBean.localNm}" />
									</c:if>
								</c:forEach>
								<c:if test="${imageFileCnt eq 0}">
									<img src="<c:url value="/resources/openworks/theme/default/images/contents/no_img.gif" />" alt="이미지 없음"/>
								</c:if>
							</c:when>
							<c:otherwise>
								<img src="<c:url value="/resources/openworks/theme/default/images/contents/no_img.gif" />" alt="이미지 없음"/>
						...
						<c:choose>
							<c:when test="${arrange.columnId eq 'TITLE'}">
								...
								<td class="tx_l">
									...
									<c:choose>
										<c:when test="${(boardConfVo.dwldYn eq 'Y') && (noticeVo.fileCnt > 0) && (noticeVo.openYn eq 'Y')}">
				...
				<td>
					<c:choose>
						<c:when test="${baseVo.fileCnt > 0}">
							<c:set var="imageFileCnt" value="0" />
							<c:forEach items="${baseVo.fileList}" var="fileBean" varStatus="status">
								<c:if test="${fn:startsWith(fileBean.fileType, 'image')}">
									<c:set var="imageFileCnt" value="${imageFileCnt + 1}" />
								</c:if>
								<c:if test="${imageFileCnt eq 1}">
					...
					<c:choose>
						<c:when test="${arrange.columnId eq 'TITLE'}">
							<td class="tx_l">
								...
								<c:choose>
									<c:when test="${(boardConfVo.dwldYn eq 'Y') && (baseVo.fileCnt > 0) && (baseVo.openYn eq 'Y')}">
