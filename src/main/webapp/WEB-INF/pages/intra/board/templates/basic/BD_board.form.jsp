...
<head>
   ...
   var jsCtgrySelPop = function(el, pid, flag) {
	   
	   if (flag == '00') {
       var goodsTyCd = '02';
       $(el).colorbox({
           title        : "서비스 카테고리선택",
           href         : "/intra/board/PD_servicePop.do",
           width        : "800",
           height       : "750",
           overlayClose : false,
           escKey       : true,
		   ...
		   var goodsTyCdArry = [];
	     var goodsTyCd = $('#goodsKndCd').val().split(",");
	     //alert(goodsTyCd[0]);
	     //var itemArry = $(this).val().split("|");
	     
	     alert(goodsTyCd.length);
/* 	     
	     for (var i==0; goodsTyCd.length>i; i++) {
	    	 goodsTyCd[i]
	     }
...
<fieldset>
	...
	<table class="boardWrite" cellspacing="0" border="0" summary="게시판 내용 작성페이지입니다.">
		...
		<tbody>
			...
			<c:if test="${boardConfVo.bbsCd == 1031}">
			<tr>
				<th><label for="basicConf">기본 설정 (2depth) <span class="tx_red tx_b">*</span></label></th>
				<td id="aditCtgList"  >
        	<select id="aditCtgCd" name="aditCtgCds" title="분류를 선택해주세요">
            <option value="">- 구분선택 2 -</option>
						<c:if test ="${not empty param.bbscttSeq}">
			...
			<tr>
				<th><label for="basicConf">서비스</label></th>
				<td>
					<input type="hidden" id="goodsNm"       name="goodsNm"      value="${dataVo.goodsNm}" /> 
					<input type="button" id="boardViewBtn"  name="boardViewBtn" value="서비스 선택" class="blue" onclick="jsCtgrySelPop(this, 'ST', '00')" />
					<input type="hidden" id="goodsCode" 		name="goodsCode"		value="${dataVo.goodsCode}"/>
			    <input type="hidden" id="goodsKndCd"		name="goodsKndCd"		value=""/>
					<table id="ctgryTitle">
						<tr>
							<td colspan="3"><div class="mar_t10 tx_b" id="test">연결된 서비스</div></td>
						</tr>
					</table>
			  	<table id="goodsCtgry">
			  		<c:set var="setGoodsNm" value="${fn:split(dataVo.goodsNm, ',')}"/>
						<c:set var="setGoodsCode" value="${fn:split(dataVo.goodsCode, ',')}"/>
						<c:set var="goodsNmCnt"   value="${fn:length(setGoodsCode)}" />
						<c:forEach var="goodsNmList" items="${setGoodsNm }" begin="0" end="${goodsNmCnt}" varStatus="goodsIdx">
						<c:forEach var="goodsCdList" items="${setGoodsCode }" begin="0" end="${goodsNmCnt}" varStatus="idx">
						<c:if test="${goodsIdx.index eq idx.index}" >
						<tr id="goodsCd_${goodsCdList }">
							<td>${goodsNmList }<input type="button" id="goodsCd_${goodsCdList}" value="X" onclick="jsCategoryDelete(this);"/></td>
						</tr>
						</c:if>
						</c:forEach>
						</c:forEach>
...
<!--          
<tr>
<th><label for="basicConf">카테고리선택</label></th>
<td>
<input type="text"   id="goodsCtgry"   name="goodsCtgry"   value="" disabled="disabled" style="width: 400px"/> 
<input type="button" value="카테고리 선택" class="blue" id="boardViewBtn" name="boardViewBtn" onclick="jsCtgrySelPop(this, 'ST', '01')" />
<input type="button" id="ctgryBtn" value="카테고리 선택" class="blue" onclick="jsCtgrySelPop(this, 'ST', '01');" />
<input type="hidden" id="goodsCtgryCd" name="goodsCtgryCd" value=""/>
   		
<table id="ctgryTitle">
<tr>
<td colspan="3"><div class="mar_t10 tx_b" id="test">연결된 카테고리</div></td>
</tr>					    
</table>
</td>
</tr>
...
<!-- 
			<tr>
				<th><label for="registPassword">비밀번호</label></th>
					<td><input type="password" name="registPassword" id="registPassword" value="${dataVo.registPassword}" /><valid:msg name="registPassword" /></td>
			</tr>
-->
			<c:forEach var="extensionVo" items="${boardConfVo.formColunms}">
			...
			<tr>
				...
				<td>
					...
					<c:if test="${not empty extensionVo.columnCm}"><p class="tx_blue_s">- ${extensionVo.columnCm}</p></c:if>
				</td>
			</tr>
			</c:if>
			</c:forEach>
				...
				<td>
					...
					<p class="tx_blue_s">- 기존 첨부파일을 <span class="tx_red">삭제</span>하시려면 아래 <span class="tx_red">X 아이콘을 클릭</span>하세요.</p>
						<ul>
							...
							<li class="mar_b5"><!-- <input type="checkbox" name="fileIds" value="${fileVo.fileId }" /><a href="/component/file/ND_fileDownload.do?id=${fileVo.fileId}">${fileVo.localNm}</a> -->
