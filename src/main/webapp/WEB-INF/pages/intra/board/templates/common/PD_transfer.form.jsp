...
<head>
	...
	<script type="text/javascript">
		$().ready(function(){
			...
			if(parent.OpenWorks.pageType == "LIST"){
			...
			$("#dataForm").validate({
				...
				submitHandler: function(form){
					...
					//상세보기일 경우 이동 액션
					else if(parent.OpenWorks.pageType == "VIEW"){
						var pform = parent.document.getElementById("dataForm");
						$("input[name=bbscttSeq]", form).val($("input[name=bbscttSeq]", pform).val());
					 	...
					 	$(form).ajaxSubmit({
							...
							data	 : {
								toBbsCd   : $("#toBbsCd1").val(),
							 	newCtg	: $("#newCtg1").val(),
							 	mngrId	 : $("input[name=mngrId]", pform).val(),
...
<form id="dataForm" name="dataForm" method="post" action="ND_article.transfer.do">
	...
	<input type="hidden" name="bbscttSeq" value="" />
