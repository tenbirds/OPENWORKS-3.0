...
<%-- 															<button type="button" class="gray_s mar_l10" onclick="jsShowFileHistory('${fileVo.fileSeq}', '${fileVo.fileId}');">이력보기</button> --%>
						...
						<c:otherwise>
							...
							<tr>
								<th><label for="${arrange.beanNm}">${arrange.columnNm}</label></th>
								<td>
									<op:bean-util field="${arrange.columnId}" obj="${dataVo}"/>
								</td>
							</tr>
							</c:if>
							...
							<tr>
								<th>
<%-- 									${arrange.beanNm}
									<label for="${arrange.beanNm}"> --%>
									<label for="${arrange.beanNm}">${arrange.columnNm}</label>
								</th>
								<td>
									<c:if test="${arrange.columnId ne 'GOODS_NM'}">
										<op:bean-util field="${arrange.columnId}" obj="${dataVo}"/>
									</c:if>
									<c:if test="${arrange.columnId eq 'GOODS_NM'}">
										<c:set var="goodsNmArry" value="${fn:split(dataVo.goodsNm, '^') }"/>
										<c:set var="goodsCdArry" value="${fn:split(dataVo.goodsCode, ',') }"/>
										...
										<c:if test="${i.index eq s.index }">
											<li><%-- <a href="javascript:jsServiceView('${goodsCode }');">${goodsName}</a> --%>
	...
	<div class="mar_t10">
		...
		<div class="float_r mar_b10">
			<c:if test="${MenuAssignType eq 'A' or MenuAssignType eq 'G'}">
			    <c:if test="${boardConfVo.bbsCd ne 1008 }">
