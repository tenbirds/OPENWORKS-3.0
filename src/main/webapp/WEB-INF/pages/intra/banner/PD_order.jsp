...
<head>
    ...
    var jsSelect = function(){
        if($('select[name="bannerTyCd"] option:selected').val() == ''){
            jsWarningBox("베너타입을 선택해주세요");
            return false;
        }
        ...
        $.ajax({
            ...
            type     : "POST",
            data: {
                bannerTyCd : $('select[name="bannerTyCd"] option:selected').val(),
            ...
            success: function(json) {
                for(var i=0; i < json.length; i++){
                    var jsonVal = json[i];
                    var domainHtml = liHtml(jsonVal.bannerTitle,jsonVal.domainCd,jsonVal.bannerSortNo,jsonVal.bannerSn,jsonVal.gubun);
    ...
    var liHtml = function(bannerTitle,domainCd,bannerSortNo,bannerSn,gubun){
        var gbn = '';
        if(gubun == '1'){
            gbn = '진행중';
        }else{
        ...
        var domainHtml = '';
            domainHtml += '<option class="sa" domainCd="'+domainCd+'" bannerSortNo="'+bannerSortNo+'" bannerSn="'+bannerSn+'">['+gbn+']&nbsp;&nbsp;&nbsp;'+ bannerTitle+'</option>';
        return domainHtml;
    };
...
<form id="dataForm" name="dataForm">
    ...
    <label for="domain">※ 사이트 기능 선택 : </label>
    <op:code id="bannerTyCd" grpCd="40" defaultValues="${param.bannerTyCd}" type="select"/>
    &nbsp;&nbsp;&nbsp;
