...
<op:jsTag type="spi" items="jquery, ui, validate, form, highlight, datepicker, colorbox"/>
...
//         var url = "ND_recomInsertAction.do";
        ...
        var recomGoodsCodes2 = new Array();
        ...
        $("input[name=recomGoodsCodes2]").each(function (i) {
            recomGoodsCodes2[i] = $(this).val();
        });
    ...
    //                        self.location.reload();
                ...
                else if (inputNm == "recomGoodsCodes2") {
	                $.ajax({
	                    url     : "ND_recomInsertAction2.do",
	                    type    : "POST",
	                    data : { goodsCodes : goodsCodes, recomendLcSeCd : lc, recomendSeCd : ty, langCode : lang, recomGoodsCodes2 : recomGoodsCodes2 },
	                    dataType: "json",
	                    success : function(result) {
	                        if (result.result) {
	                            jsSuccessBox(result.message);
    ...
    var jsRecomDelete = function(inputNm) {
        if (inputNm == "recomGoodsCodes1") {
	        ...
	        $("input[name=goodsCodes2]:checked").each(function (i) {
	            $("input[name=" + inputNm + "][value=" + this.value + "]").remove();
	        });
        }
        $("#dataForm").submit();
    };
//     var jsRecomDelete = function(lc, ty) {
//          var url = "ND_recomDeleteAction.do";
//          var goodsCodes = new Array();
//          var tyMsg;
...
//             if (ty == 1003) {
//                 $("input[name=goodsCodes3]:checked").each(function (i) {
//                       goodsCodes[i] = $(this).val();
//                 });
//             }
...
//              if(confirm("[ " + tyMsg + " ] 에서 선택한 " + goodsCodes.length + "개의 항목을 해제 하시겠습니까?")){
//                  jQuery.ajaxSettings.traditional = true;

//                  $.ajax({
//                      url     : "ND_recomDeleteAction.do",
//                      type    : "POST",
//                      data : { goodsCodes : goodsCodes, recomendLcSeCd : lc, recomendSeCd : ty },
//                      success : function(result) {
...
//     };
	...
	//  var url = "ND_recomInsertAction.do";
        ...
        var priort2 = new Array();
        var appnDt1 = new Array();
        var appnDt2 = new Array();
        ...
        $("input[name=recomGoodsCodes2][value!=NONE]").each(function (i) {
            recomGoodsCodes2[i] = $(this).val();
        });
        ...
        $("select[name=priort2]").each(function (i) {
        	priort2[i] = $(this).val();
        });
        ...
        $("input[name=appnDt2]").each(function (i) {
            appnDt2[i] = $(this).val();
        });

        if (inputNm == "recomGoodsCodes1") {
            tyMsg = "추천 표준프레임워크호환 서비스를";
        } else if (inputNm == "recomGoodsCodes2") {
            tyMsg = "추천 솔루션/서비스를";
        }
        if(confirm(tyMsg + " 적용 하시겠습니까?")){
           jQuery.ajaxSettings.traditional = true;
           if (inputNm == "recomGoodsCodes1") {
               $.ajax({
               ...
               $.ajax({
                   url     : "ND_recomInsertApply2.do",
                   type    : "POST",
                   data : { goodsCodes : recomGoodsCodes2, recomendLcSeCd : lc, recomendSeCd : ty, langCode : lang, priorts : priort2, appnDts : appnDt2 },
                   dataType: "json",
                   success : function(result) {
                       if (result.result) {
                           jsSuccessBox(result.message);
...
//      var jsApply = function (inputNm) {
//          if (limitCnt < $("input[name=" + inputNm + "]").length + $("input[name=goodsCodes]:checked").length)) {
//              alert("지정된 갯수가 초과되었습니다.");
//          }
//          $("input[name=recomGoodsCodes2][value!=NONE]").each(function (i) {
//              alert(this.value);
//              $("#"+inputNm).html("<input type=\"hidden\" name=\"" + inputNm + "\" value=\"" + this.value + "\"/>");
//          });
//          $("#dataForm").submit();
//      }
     ...
     var jsLangChange = function (langCode) {
         $("#q_init").val("0");
         self.location.href = "BD_index.do?q_langCodes="+langCode;
     };
...
<%--             <input type="hidden" name="langCode" id="langCode" value="${param.q_langCodes}"/> --%>
            <span id="recomGoodsCodes2">
                <input type="hidden" name="recomGoodsCodes2" value="NONE"/>
            </span>
            ...
            <ul style="margin-bottom: 0px;">
                <li id="infoTab1" style="width:200px;"><a href="javascript:;"><span>메인 추천서비스</span></a></li>
                ...
                <li id="infoTab4" style="width:200px;"><a href="javascript:;" onclick="jsTab('BD_recomConsult.do');"><span>IT전문가서비스 추천서비스</span></a></li>
            </ul>
            </div>
            ...
            <div class="mar_t20">
            <h4>추천솔루션 (최대 3개)</h4>
            <table class="boardList" cellspacing="0" border="0" summary="추천솔루션">
            ...
            <tbody>
                <c:set var="index" value="${recomMainS.indexNo}"/>
                <c:forEach items="${recomMainS.list}" var="baseVo" varStatus="status">
                    <tr>
                        <td><input type="checkbox" name="goodsCodes2" value="${baseVo.goodsCode}"/>
                        <input type="hidden" name="recomGoodsCodes2" value="${baseVo.goodsCode}"/>
                        </td>
                        <td>
                        	<select name="priort2">
                        ...
                        <td><input type="hidden" name="appnDt2" value="${baseVo.appnDt}"/>${baseVo.appnDt}</td>
                    </tr>
                </c:forEach>
                <op:no-data obj="${recomMainS}" colspan="10" />
            </tbody>
            </table>
            <div>
                  <button type="button" class="blue mar_t5" onclick="jsRecomDelete('recomGoodsCodes2');">추천솔루션 해제</button>
                  <button type="button" class="blue mar_t5 float_r" onclick="jsApply(1001, 1002, 'recomGoodsCodes2');">추천솔루션 적용</button>
            </div>
            </div>

            <div class="mar_t20">
            <h4>표준프레임워크호환 서비스 (최대 4개)</h4>
            <table class="boardList" cellspacing="0" border="0" summary="표준프레임워크호환 서비스">
            <caption class="hidden"> 목록 </caption>
            <colgroup>
                <col width="3%"/>
                <col width="5%"/>
            ...
            <tbody>
                <c:set var="index" value="${recomMainP.indexNo}"/>
                <c:forEach items="${recomMainP.list}" var="baseVo" varStatus="status">
                    <tr>
                ...
                <op:no-data obj="${recomMainP}" colspan="10" />
            </tbody>
            </table>
            <div>
<!--             <button type="button" class="blue mar_t5" onclick="jsRecomDelete(1001, 1001);">표준프레임워크호환 서비스 해제</button> -->
                  <button type="button" class="blue mar_t5" onclick="jsRecomDelete('recomGoodsCodes1');">추천 표준프레임워크호환 서비스 해제</button>
                  <button type="button" class="blue mar_t5 float_r" onclick="jsApply(1001, 1001, 'recomGoodsCodes1');">추천 표준프레임워크호환 서비스 적용</button>
            </div>
            </div>
...
<!--         <form name="dataForm2" id="dataForm2" method="post"> -->
        ...
        <div>
	        <div class="tx_c">
	             <a href="javascript:;" onclick="jsRecom(1001, 1002, 'recomGoodsCodes2');"><button class="w_blue mar_r10" type="button">선택항목 추천 솔루션</button></a>
	             <a href="javascript:;" onclick="jsRecom(1001, 1001, 'recomGoodsCodes1');"><button class="w_blue mar_l10" type="button">선택항목 추천 표준프레임워크호환 서비스</button></a>
	        </div>
        </div>
        <!--  //버튼 -->
    </form>
