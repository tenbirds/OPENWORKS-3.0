    ...
    $().ready(function () {
        $("#dataForm").validate({
            rules: {
                ...
                q_beginConfmDate : { dateISO: true, date: true },
                q_endConfmDate : { dateISO: true, date: true }
            },
            messages: { 
                ...
                q_beginConfmDate: {dateISO: "시작날짜가 형식에 맞지 않습니다.",
                   					date: "시작날짜가 형식에 맞지 않습니다."},
                q_endConfmDate: {dateISO: "시작날짜가 형식에 맞지 않습니다.",
                       			  date: "시작날짜가 형식에 맞지 않습니다."}
                 },
            submitHandler : function(form) {
                ...
                if ($("#q_beginConfmDate").val()=='' &&  $("#q_endConfmDate").val()!='') {
                    alert("시작날짜가 존재하지 않습니다.");
                    return false;
                }
                if ($("#q_beginConfmDate").val()!='' &&  $("#q_endConfmDate").val()=='') {
        ...
        $("span#confmPeriod  a  button.gray").click(function(){
            var sType = $(this).attr('id');
            var q_beginDate = null;
            var q_endDate = null;
            
            if ($("#q_beginConfmDate").val()==''){
                $('#q_endConfmDate').val(getDateStr(new Date()));
            }
            if (! $("#q_endConfmDate").dateValidator()) {
                alert("종료날짜가 형식에 맞지 않습니다.");
                return false;
            }
            switch(sType){
                case '0' :
                    q_beginDate = new Date();
                    $('#q_beginConfmDate').val(getDateStr(q_beginDate));
                    q_endDate = new Date();
                    $('#q_endConfmDate').val(getDateStr(q_endDate));
                    break;
                case '7D':
                    q_beginDate = getCalDate(new Date($("#q_endConfmDate").val()),0,-7);
                    break;
                case '1M':
                    q_beginDate = getCalDate(new Date($("#q_endConfmDate").val()),-1,0);
                    break;
                case '3M':
                    q_beginDate = getCalDate(new Date($("#q_endConfmDate").val()),-3,0);
                    break;
                default:
                    $("#q_beginConfmDate").val("2000-01-01");
                    $("#q_endConfmDate").val("2100-12-31");
                    break;
            }
            $('#q_beginConfmDate').val(getDateStr(q_beginDate));
        ...
        $("#q_beginDate, #q_beginConfmDate").keyup(function(event){
            if (!(event.keyCode >=37 && event.keyCode<=40)) {
                var inputVal = $(this).val();
                $(this).val(inputVal.replace(/[^a-z0-9\-]/gi,''));
            }
        });
        $("#q_endDate, #q_endConfmDate").keyup(function(event){
            if (!(event.keyCode >=37 && event.keyCode<=40)) {
                var inputVal = $(this).val();
                $(this).val(inputVal.replace(/[^a-z0-9\-]/gi,''));
            }
...
<!-- 기능별 스크립트 정의 -->    
    ...
    var jsSearch = function(){
        ...
        $("#dataForm").attr("action", "BD_goodsRequstList.do");
        $("#dataForm").submit();
    };
    ...
    var jsCtgryList = function(el ,vl){
	    ...
	    var jsDspy = function(dspyAt) {
	        ...
	        if(goodsCodePk.length == 0){
	            ...
	            if(confirm("선택한 " + goodsNm + " 항목을 승인완료 처리 하시겠습니까?")){
     ...
     var jsContentDiffView = function(el, goodsCode, goodsKndCd, exmntConfmHistSn, gubun){

       	$(el).colorbox({
           	title : "사용자 정보",
           	//href  : "/intra/orderManage/purchs/PD_tempUserView.do?goodsCode=" + goodsCode,
           	href  : "BD_goodsFormPopUp.do?goodsCode="+goodsCode+"&goodsKndCd="+goodsKndCd+"&exmntConfmHistSn="+exmntConfmHistSn+"&gubun="+gubun,
           	width : "1250",
           	height: "500",
         	iframe: true
     	});
...
<body>
    <div id="contents">
        <!-- 검색 -->
        <form name="dataForm" id="dataForm" method="get" action="BD_goodsRequstList.do">
          <input type="hidden" name="q_init" 	  id="q_init" 	 value="${param.q_init}"/>
          <input type="hidden" name="langCodes" id="langCodes" value="" />
          <input type="hidden" name="requstProcessAt" value="M"/>
                    ...
                    <table class="boardWrite" cellspacing="0" border="0" summary="검색 조건">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <th>승인일</th>
                                <td colspan="3">
                                    <input class="w120" type="text" name="q_beginConfmDate" id="q_beginConfmDate" title="시작날짜선택" maxlength="10"<c:if test="${not empty param.q_beginConfmDate}"> value="${param.q_beginConfmDate}"</c:if>/> ~
                                    <input class="w120" type="text" name="q_endConfmDate" id="q_endConfmDate" title="종료날짜선택" maxlength="10"<c:if test="${not empty param.q_endConfmDate}"> value="${param.q_endConfmDate}"</c:if>/>
                                    <span class="mar_l10" id="confmPeriod">
                                        <a href="javascript:;"><button type="button" id="0"   class="gray">오늘</button></a>
                                        <a href="javascript:;"><button type="button" id="7D"   class="gray">일주일</button></a>
                                        <a href="javascript:;"><button type="button" id="1M"   class="gray">1개월</button></a>
                                        <a href="javascript:;"><button type="button" id="3M"   class="gray">3개월</button></a>
                            ...
                            <tr>
                                ...
                                <td colspan="3">
                                    <select id="q_searchKey" name="q_searchKey" title="상세검색을 선택하세요"> 
                                        ...
                                        <option value="1003">신청기업명</option>
                                        <option value="1004">신청자명</option>
                                    </select>
        ...
        <op:pagerParam title="서비스 비전시.삭제관리 목록" />
    	</form> 
        <!-- //검색 -->
        <!-- 리스트 -->
        <table class="boardList" cellspacing="0" border="0" summary="리스트입니다.">
            ...
            <tbody id="odd-color">
                ...
                <c:forEach items="${pager.list}" var="baseVo" varStatus="status">
                    <tr>
                      <td> <c:if test="${baseVo.requstProcessAt eq 'N'}"> 
                        ...
                        <td id="${baseVo.goodsCode}${baseVo.exmntConfmHistSn}-goodsNm" class="tx_l"><a href="javascript:;" onclick="jsContentDiffView(this,'${baseVo.goodsCode}','${baseVo.goodsKndCd}','${baseVo.exmntConfmHistSn}','Y');">${baseVo.goodsNm}</a>
                        ...
                        <td> ${baseVo.goodsChargerNm}<c:if test="${baseVo.goodsChargerNm eq null}">관리자</c:if></td>
                        ...
                        <td>
                        <c:if test="${baseVo.requstProcessAt eq 'Y' or (baseVo.exmntConfmCd eq 1007  and baseVo.goodsChngSeq> 0) }">${baseVo.exmntConfmDt}</c:if>
                        </td>
                        <td>
                        <c:if test="${baseVo.requstProcessAt eq 'Y' or (baseVo.exmntConfmCd eq 1007  and baseVo.goodsChngSeq> 0) }">${baseVo.mngrId}</c:if>
                        </td>
                    </tr>
                </c:forEach>
        ...
        <div>
	        <div class="float_l">
	            <!-- <a href="javascript:;" onclick="jsDspy('M');"><button class="w_blue" type="button">검토</button></a> -->
	            <a href="javascript:;" onclick="jsDspy('M');"><button class="w_blue" type="button">선택항목 승인</button></a>
	        </div>
        </div>
        <!--  //버튼 -->
    </div>
