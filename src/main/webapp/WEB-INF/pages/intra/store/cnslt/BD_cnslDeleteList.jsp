...
//        document.dataForm.action = "BD_cnslDeleteList.do";
        document.dataForm.elements["q_currPage"].value = "1";
        document.dataForm.elements["q_init"].value = "0";
//         document.dataForm.submit();
        $("#dataForm").attr("action", "BD_cnslDeleteList.do");
        $("#dataForm").submit();
    };
	        ...
	        if(goodsCodes.length == 0){
	            ...
	            if(confirm("선택한 " + goodsCodes.length + "개의 항목을 " + ActvtyMsg + " 하시겠습니까?")){
	                ...
	                $.ajax({
	                    ...
	                    type    : "POST",
	                    data : { goodsCodes : goodsCodes, goodsRegistSttus : dspyAt },
                                    ...
                                    <p class="mar_t5" id="langCheck">
                                        ...
                                        <c:forEach items="${codeList}" var="_bean" varStatus="status">
                                            <input type="checkbox" name="q_langCodes" id="q_langCode_${status.count}"
                                                <c:if test="${fn:contains(param.q_langCodes,_bean.langCode)}">checked="checked"</c:if>
...
<!--                                 </td> -->
            ...
            <op:pagerParam title="IT전문가서비스 삭제 목록" />
...
<!--                         <td> -->
<%--                            <c:forEach items="${baseVo.markFile}" var="fileVo" varStatus="status">                            --%>
...
<%--                             ${baseVo.sleMthdNm} --%>
<!--                         </td>                         -->
                        <td><a href="javascript:;" onclick="jsStoreView(this,'${baseVo.userId}')"><span > ${baseVo.langStoreNm} </span><br />(${baseVo.userId})</a></td>
                        <td><c:if test="${baseVo.mngrDeleteAt=='Y'}">관리자</c:if><c:if test="${goodsActvtyAt=='N'}">판매자</c:if></td>
                        <td>${baseVo.updtDt}</td>
                    </tr>
                </c:forEach>
