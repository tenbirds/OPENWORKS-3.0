...
<head>
    ...
    <script type="text/javascript">
        ...
        var jsErrorView = function(errorNo) {
            ...
            jsView("BD_TransactorErrorView.do?q_errorNo="+errorNo);
        };
...
<body>
    ...
    <ul class="tab">
        <c:if test="${not empty SUB_SUPER}">
            <li><a href="#" onclick="jsErrorTabList('BD_TransactorErrorList.do', 1010);return false;" <c:if test="${empty param.q_errorProcStatus or param.q_errorProcStatus eq '1010'}" >class="on"</c:if>>미배정 (${baseVo.stat01})</a></li>
        </c:if>
        <li><a href="#" onclick="jsErrorTabList('BD_TransactorErrorList.do', 1020);return false;" <c:if test="${empty SUB_SUPER  and empty param.q_errorProcStatus or param.q_errorProcStatus eq '1020'}" >class="on"</c:if>>배정 (${baseVo.stat02})</a></li>
        <li><a href="#" onclick="jsErrorTabList('BD_TransactorErrorList.do', 1030);return false;" <c:if test="${param.q_errorProcStatus eq '1030'}" >class="on"</c:if>>처리중 (${baseVo.stat03})</a></li>
        <li><a href="#" onclick="jsErrorTabList('BD_TransactorErrorList.do', 1040);return false;" <c:if test="${param.q_errorProcStatus eq '1040'}" >class="on"</c:if>>완료 (${baseVo.stat04})</a></li>
        <li><a href="#" onclick="jsErrorTabList('BD_TransactorErrorList.do', 1000);return false;" <c:if test="${param.q_errorProcStatus eq '1000'}" >class="on"</c:if>>전체 (${baseVo.stat05})</a></li>
    </ul>

    <!-- 검색 -->
    <form name="dataForm" id="dataForm" method="get" action="BD_TransactorErrorList.do">
        ...
        <c:choose>
            <c:when test="${not empty SUB_SUPER or param.q_errorProcStatus eq '1010'}"><c:set var="assignStatus" value="미배정"/></c:when>
            <c:when test="${empty param.q_errorProcStatus or param.q_errorProcStatus eq '1020'}"><c:set var="assignStatus" value="배정"/></c:when>
    ...
    <c:choose>
        <c:when test="${not empty SUB_SUPER}">
            <c:choose>
                <c:when test="${empty param.q_errorProcStatus or param.q_errorProcStatus eq '1010'}"><c:set var="cols" value="6"/></c:when>
        ...
        <c:otherwise>
            <c:choose>
                <c:when test="${empty param.q_errorProcStatus or param.q_errorProcStatus eq '1020'}"><c:set var="cols" value="7"/></c:when>
                <c:when test="${param.q_errorProcStatus eq '1030'}"><c:set var="cols" value="8"/></c:when>
                <c:when test="${param.q_errorProcStatus eq '1040'}"><c:set var="cols" value="9"/></c:when>
                <c:otherwise><c:set var="cols" value="10"/></c:otherwise>
            </c:choose>
    ...
    <table class="boardList" cellspacing="0" border="0" summary="오류관리 목록">
        ...
        <thead>
            <tr>
                ...
                <th <c:if test="${empty param.q_errorProcStatus or param.q_errorProcStatus eq '1010'}">class="lr_none"</c:if>><span>최초발생일<br />최종발생일</span></th>
        ...
        <tbody>
            ...
            <c:forEach items="${pager.list}" var="dataVo" varStatus="status">
                ...
                <tr ${rowCss}>
                    ...
                    <td class="tx_l <c:if test="${empty param.q_errorProcStatus or param.q_errorProcStatus eq '1010'}">lr_none</c:if>">
                        ${dataVo.errorStDt} ~
                        <br />
                        ${dataVo.errorEdDt}
