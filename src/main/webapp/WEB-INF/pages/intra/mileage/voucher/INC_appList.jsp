...
$().ready(function() {
    $('#subTab1').attr("class", "");
    $('#subTab2').attr("class", "on");
    $('#subTab3').attr("class", "");
    $('#subTab4').attr("class", "");
    $('#q_searchPrcType').attr("display","none");
	...
	<table class="boardList" cellspacing="0" border="0" summary="리스트">
		...
		<c:forEach var="item" items="${pager.list}" varStatus="status">
			<tr <c:if test="${(index-status.index)%2==0}">class="bg_yellow"</c:if>>
				...
				<td>
					<a href="#" onclick="sndStatusView(this, '${item.seqNo}', '${item.userId}');">
		...
		<op:no-data obj="${pager.list}" colspan="11" />
		</tbody>
	</table>
	
	<p class="mar_t10">※ 각 현재 마일리지 점수를 클릭하시면 마일리지 적립 내역을 확인할 수 있습니다.</p>
	<p class="mar_t10">※ 각 승인여부를 클릭 하시면 발송 처리/미처리를 선택 할 수 있습니다.</p>
	<!-- //리스트 -->
