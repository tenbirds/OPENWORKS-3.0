...
<script type='text/javascript'>
    ...
    var customerInit = function() {
        ...
        $('tbody.chartList tr').each(function (index) {
            data[index] = { name  : $(this).find('td').eq(0).text().replace(/\s/g, ''),
                            data0 : parseInt($(this).find('td').eq(3).text(), 10),
                            data1 : parseInt($(this).find('td').eq(6).text(), 10),
                            data2 : parseInt($(this).find('td').eq(9).text(), 10)
            };
        });
        initSort();
        initChartMutilBar();
    ...
    var reportAction = function(){
        ...
        var open = "/report/reporting.jsp?reportSection=1004&q_langCodes="+q_langCodes+"&q_beginDate="+q_beginDate+"&q_endDate="+q_endDate;
...
<body>
    <div class="search">
        <fieldset>
            ...
            <table class="boardWrite" cellspacing="0" border="0" summary="광고/배너 검색">
    ...
    <table class='boardList' cellspacing='0' border='0' summary='리스트입니다.'>
        ...
        <thead id='listHead'>
            <tr>
                <th rowspan="2">구분<br/>(광고주)</th>
                <th colspan="3">팝업</th>
                <th colspan="3">팝업존</th>
                <th colspan="3" class="lr_none">배너</th>
            </tr>
            <tr>
                <th>노출수</th>
                <th>클릭수</th>
        ...
        <tbody id='listBody' class='chartList'>
            <c:forEach items='${dataList}' var='data' varStatus='idx'>
                <tr <c:if test='${(idx.count mod 2) eq 0}'> class='bg_yellow'</c:if>>
                    <td><c:out value='${data.AVTS_NM}'/></td>
                    <td><c:out value='${data.POP_SHOW_CNT}'/></td>
                    <td><c:out value='${data.POP_CLICK_CNT}'/></td>
                    <td><c:out value='${data.POP_CTR}'/>%</td>
                    <td><c:out value='${data.ZON_SHOW_CNT}'/></td>
                    <td><c:out value='${data.ZON_CLICK_CNT}'/></td>
                    <td><c:out value='${data.ZON_CTR}'/>%</td>
                    <td><c:out value='${data.BAN_SHOW_CNT}'/></td>
                    <td><c:out value='${data.BAN_CLICK_CNT}'/></td>
                    <td class="lr_none"><c:out value='${data.BAN_CTR}'/>%</td>
                </tr>
            </c:forEach>
            <c:if test="${empty dataList}">
                <tr>
                    <td colspan="10" class="lr_none">데이터가 존재하지 않습니다.</td>
                </tr>
            </c:if>
        </tbody>
    </table>
