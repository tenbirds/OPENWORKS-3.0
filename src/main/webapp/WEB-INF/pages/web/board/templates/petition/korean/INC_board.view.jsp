...
<%@ taglib uri="http://www.openworks.kr/jsp/validate" prefix="valid"%>
    ...
    <div class="contents">
					<!-- section -->
					<c:if test="${dataVo.bbsCd ne '1030'}">
					<div class="section">
					<!-- 답변 달기 -->
				
			        <c:if test="${not empty dataVo.replyDt and not empty dataVo.replyCn }">
			            <table class="tbl_st2" summary="게시글 상세">
			            	<caption>
				                    ...
				                    <c:choose>
				                        ...
				                        <c:when test="${arrange.columnId eq 'REGIST_ID' or arrange.columnId eq 'OTHBC_AT'}">
				                            <c:if test="${arrange.columnId eq 'REGIST_ID'}"><!-- 등록자명 -->
				                                <tr>
				                                    <th class="tit" scope="col">작성자ID</th>
				                                    <c:choose>
				                            ...
				                            <c:if test="${arrange.columnId eq 'OTHBC_AT'}"><!-- 공개여부 -->
				                                    <th class="tit" scope="col">공개여부</th>
				                                    <td class="last">
				                        ...
				                        <c:when test="${arrange.columnId eq 'FILE_SEQ'}">
				                            ...
				                            <c:if test="${!empty dataVo.fileList}">
				                                <tr>
				                                    <th class="tit" scope="col"><label for="${arrange.beanNm}">첨부파일</label></th>
				                        ...
				                        <c:otherwise>
				                            <!-- 기타 항목 -->
				                            <tr>
				                                <th  scope="col"><label for="${arrange.beanNm}">${arrange.columnNm}</label></th>
