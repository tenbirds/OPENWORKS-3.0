...
//         document.getElementById(span).innerHTML = "<em>" + li_char + "</em>" + "/" + li_max;
    ...
    var jsCtrgyGoodsUserPop = function(){
        if(OpenWorks.pageType == "UPDATE"){
	        if(ctgryDate == null || ctgryDate == ""){//새로운카테고리로 수정한경우
	            ctgryCodes = $("#setCtgryCodes").val();
	        } else{
    ...
    var jsCtgrySel = function() {
		...
		if (ctgryCodes == "") {
            ...
            htmlVal = '<tr>';
            htmlVal += '<th class="requ">연결된 카테고리</th>';
            htmlVal += '<td>';
            htmlVal += '<a href="#" onclick="jsCtgrySelPop(this);" class="btns st3">카테고리 선택</a>';
            htmlVal += '※ 카테고리 선택은 최대 3개 까지 가능합니다.';
            htmlVal += '<p class="desc"> </p>';
            htmlVal += '</td>';
            htmlVal += '</tr>';
        }else {
            ...
            $.post(url, function(response){
                if(response.length > 0){
                    ...
                    for (var i = 0; i < response.length; i++) {                         
                        ...
                        htmlVal += '['+response[i].ctgryClNm+']'+ response[i].ctgryPath;
		...
		// 카테고리가 변경 시 전문가 선택 초기화
		$("#viewCtgryUserNms").text("");
		$("#estnColumn2").val(""); 
  		$("#estnColumn1").val(""); 
	};
	...
	/* 201612268 전문가선택 팝업 */  
	var jsCtgryUserSelPop = function(el) {
	    
	    var ctgryCodes = $("#setCtgryCodes").val();	
 		
	    if (ctgryCodes == "") {	 
		    alert("최소 1개 이상의 카테고리를 선택해야합니다.");
		    return;
		}		    
	    var ctgryCd1 = "NULL";
	    var ctgryCd2 = "NULL";
	    var ctgryCd3 = "NULL";
 		...
 		for (var i = 0; i < ctgryCodes.length; i++) {    
             ...
             if( i == 0){
                 ctgryCd1 = ctgryCodes[i]
             } else if( i == 1){
                 ctgryCd2 = ctgryCodes[i] 
             } else {
                 ctgryCd3 = ctgryCodes[i]
             }   
         } 

	    $(el).colorbox({
	          title 		 : "전문가 선택",
	          href 			 : "/korean/pt/popup/PD_categoryUserPop.do?columnVal1="+ctgryCd1+"&columnVal2="+ctgryCd2+"&columnVal3="+ctgryCd3,
 	          width 		 : "680",
	          height 		 : "400",
	          overlayClose   : false,
	          escKey 		 : true,
