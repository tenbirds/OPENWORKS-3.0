					...
					<td>
						<c:choose>
							<c:when test="${(pageType eq 'VIEW') && (noticeVo.bbscttSeq eq dataVo.bbscttSeq)}">
								<img src="<c:url value='/resources/openworks/theme/default/images/icon/icon_current.png' />" alt="" />
							</c:when>
							<c:otherwise>
								<img src="<c:url value='/resources/openworks/theme/default/images/icon/icon_notice2.gif' />" alt="" />
							</c:otherwise>
						</c:choose>
					</td>
					...
					<c:forEach items="${boardConfVo.listArrange}" var="arrange" varStatus="arrStatus">
						<c:choose>
							...
							<c:when test="${arrange.columnId eq 'ESTN_COLUMN5'}">
	                            ...
	                            <td>
	                               <c:if test="${boardConfVo.estnColumn5 eq 'Y'}"><span >선정</span></c:if>
	                               <c:if test="${boardConfVo.estnColumn5 eq 'N'}"><span>미선정</span></c:if>
	                               <c:if test="${boardConfVo.estnColumn5 eq 'S'}"><span>대기</span></c:if>
	                            </td>
	                            <!-- 상태여부 -->
