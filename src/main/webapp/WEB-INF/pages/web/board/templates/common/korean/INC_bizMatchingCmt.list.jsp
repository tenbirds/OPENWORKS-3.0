...
//            toobig: "$file 은 크기가 매우 큽니다. (max $size)",
            ...
            if(result.result == true) {
                ...
                if(result.value.fileList.length > 0){
                    ...
                    for(i = 0; i < result.value.fileList.length; i++){
                        ...
                        innerHtml += '<a href="#" onclick="jsFileDelete(this, &#39;' + result.value.fileList[i].fileSeq + '&#39;,&#39;' + result.value.fileList[i].fileId + '&#39;);"><img src="<c:url value="/resources/openworks/theme/default/images/btn/btn_r_close.gif" />" alt="삭제" class="vm"/></a>';
    ...
    var jsSubmit = function(){
        ...
        /* if(!confirm("비즈매칭 선정시 대표전화가 비즈매칭 요청자에게 공개됩니다.\n등록하시겠습니까?")) */
        if(!confirm("등록하시겠습니까?"))
            return false;

        if("${__usk.userKey}" == "${ cmtBoardVo.userKey}"){
            //본인의 글에는 글등록자가 댓글등록이 불가능
            alert("본인이 등록한 글에는 답변이 불가능합니다.");
            return false;
        }
...
<!-- 의견글내용-->
				...
				<c:forEach items="${pagerVo.list}" var="commentVo" varStatus="status">
					...
					<c:choose>
	                    <c:when test="${commentVo.mngrDeleteAt eq 'Y'}">
							<table class="tbl_st4">
								<tr>
									<td colspan="4">관리자에 의해 삭제되었습니다.</td>
								</tr>
							</table>
						</c:when>
						<c:when test="${commentVo.bizMatchingOthbcAt eq 'N' and (!empty __usk.userKey && (__usk.userKey eq commentVo.userKey or __usk.userKey eq cmtBoardVo.userKey ))}">
							...
							<table class="tbl_st4">
								...
								<tr>
									<th>답변자</th>
									<td colspan="3">${commentVo.userNm}</td>
								</tr>
								<tr>
									<th>내용</th>
									<td colspan="3"><op:nrToBr content='${commentVo.nttCmtCn}'/></td>
								</tr>
								<c:choose>
	                            ...
	                            <tr>
									<th>작성일</th>
									<td colspan="3">
										<fmt:parseDate value='${commentVo.registDt}' var='registDt' pattern="yyyy-MM-dd HH:mm" scope="page"/>
										<fmt:formatDate value="${registDt}" pattern="yyyy-MM-dd"/>
										<c:if test="${empty cmtBoardVo.bizCmtSeq and commentVo.mngrDeleteAt != 'Y'}">
		                            		<c:if test="${!empty __usk.userKey && __usk.userKey eq commentVo.userKey}">
		                                		<c:if test="${empty cmtBoardVo.bizCmtSeq and cmtBoardVo.bizCmtSeq ne commentVo.cmtSeq}">
												 | <a href="javascript:;" onclick="jsModifyCommentAction('<c:out value='${commentVo.domainCd}'/>','<c:out value='${commentVo.bbsCd}'/>', '<c:out value='${commentVo.bbscttSeq}'/>', '<c:out value='${commentVo.cmtSeq}'/>', '<c:out value='${commentVo.mngrId}'/>', '<c:out value='${commentVo.registId}'/>');">수정</a>
												 | <a href="javascript:;" onclick="jsDeleteCommentAction('<c:out value='${commentVo.domainCd}'/>','<c:out value='${commentVo.bbsCd}'/>', '<c:out value='${commentVo.bbscttSeq}'/>', '<c:out value='${commentVo.cmtSeq}'/>', '<c:out value='${commentVo.mngrId}'/>', '<c:out value='${commentVo.registId}'/>');">삭제</a>
												</c:if>
		                            		</c:if>
		                        		</c:if>
									</td>
			...
			<!-- 의견글등록-->
			<c:if test="${(sessionScope['__usk'].userTyCd eq '1002' || sessionScope['__usk'].userTyCd eq '2002' || sessionScope['__usk'].userTyCd eq '3002') && __usk.userKey ne cmtBoardVo.userKey}" >
			<c:if test="${cmtBoardVo.bizProgression eq 'PRO'}">
		        ...
		        <input type="hidden" id="bizMatchingEmail" name="bizMatchingEmail" title="이메일 입력" value="${dataVo2.entrprsCmpnyEmail}"/>
                <h3>전문가 답변 쓰기</h3>
				<table class="tbl_st4 e4">
					<tr>
						<th>회사명</th>
						<td>
							<input type="hidden" id="bizMatchingCmpnyNm" name="bizMatchingCmpnyNm" title="회사명 입력" value="${dataVo2.entrprsCmpnyNm}"/>${dataVo2.entrprsCmpnyNm}
						</td>
						<th>대표전화</th>
						<td class="tel">
				            <input type="text" id="bizMatchingCttpc1" name="bizMatchingCttpc1" title="대표전화번호 입력" value="${fn:split(dataVo2.reprsntTelno,'-')[0]}" maxlength="4"/><span class="space">-</span>
                            <input type="text" id="bizMatchingCttpc2" name="bizMatchingCttpc2" title="대표전화번호 입력" value="${fn:split(dataVo2.reprsntTelno,'-')[1]}" maxlength="4"/><span class="space">-</span>
                            <input type="text" id="bizMatchingCttpc3" name="bizMatchingCttpc3" title="대표전화번호 입력" value="${fn:split(dataVo2.reprsntTelno,'-')[2]}" maxlength="4"/>
						</td>
					</tr>
					<tr>
						<th>답변자</th>
						...
						<th>공개여부</th>
						<td>
							<label><input type="checkbox" type="checkbox" id="bizMatchingOthbcAt" name="bizMatchingOthbcAt" value="Y"/> 공개</label>
						</td>
					</tr>
					<tr>
						<th>내용</th>
						<td colspan="3">
							<textarea id="nttCmtCn" name="nttCmtCn" title="내용 입력" placeholder="장시간 내용 작성시 세션이 끊겨 작성중인 내용이 사라질 수 있으니 주의하시기 바랍니다."></textarea>
						</td>
					</tr>
					<tr>
						<th>첨부파일</th>
						<td colspan="3">
								...
								<input type="hidden" id="uploadFileCnt" value="<c:out value='${cmtBoardVo.fileCnt}' />" />
	                            ...
	                            <div id="fileView" class="regist-file"></div>
	                            <!-- //file upload -->
				            <p class="desc">
				            	<c:if test="${fn:length(boardConfVo.filePermExtsn) >= 3 }">
	                                ...
	                                ※${join_filePermExtsn} 파일 첨부 가능<br />
	                            </c:if>
				...
				<div class="btnArea rig">
					<a href="#" onclick="jsSubmit();" class="btns st1 icn icnSave">저장</a>
				</div>
			    </form>
			</c:if>
			</c:if>
