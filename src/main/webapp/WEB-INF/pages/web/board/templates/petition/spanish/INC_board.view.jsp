                    ...
                    <c:choose>
                        ...
                        <c:when test="${arrange.columnId eq 'REGIST_ID' or arrange.columnId eq 'OTHBC_AT'}">
                            ...
                            <c:if test="${arrange.columnId eq 'OTHBC_AT'}"><!-- 공개여부 -->
                                    ...
                                    <td class="last">
                                        <c:if test="${dataVo.othbcAt eq 'N'}"> Oculto</c:if>
                                        <c:if test="${dataVo.othbcAt eq 'Y'}"> Divulgación</c:if>
                                    </td>
                                </tr>
                            </c:if>
                        </c:when>
                        <c:when test="${arrange.columnId eq 'REGIST_DT' or arrange.columnId eq 'ESTN_COLUMN3'}">
                            <c:if test="${arrange.columnId eq 'REGIST_DT'}"><!-- 등록일 -->
                                <tr>
                                    <td class="tit">Fecha de publicación</td>
                                    <td><c:out value='${dataVo.registDt}'/></td>
                            </c:if>
                            <c:if test="${arrange.columnId eq 'ESTN_COLUMN3'}"><!-- 상태 -->
                                    <td class="tit">Condicion</td>
                                    <td class="last">
                                        <c:if test="${dataVo.estnColumn3 eq 'N'}">Respuesta incompleta</c:if>
                                        <c:if test="${dataVo.estnColumn3 eq 'Y'}">Respuesta completa</c:if>
                                    </td>
                                </tr>
                            </c:if>
                        </c:when>
                        ...
                        <c:when test="${arrange.columnId eq 'FILE_SEQ'}">
                            ...
                            <c:if test="${!empty dataVo.fileList}">
                                <tr>
                                    <td class="tit"><label for="${arrange.beanNm}">Archivo adjunto</label></td>
                        ...
                        <td colspan="2">
                            <div class="tag">
                                <c:if test="${!empty dataVo.bbsTags}">
                                    <img src="/resources/openworks/theme/default/images/icon/icon_tag.gif" alt="etiquetas" class="vm" />
