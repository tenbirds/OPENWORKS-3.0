...
<!-- 사용자 스크립트 끝 -->
    ...
    <div id="commentReplyModify" class="none">
		...
		<div class="textarea">
			...
			<label for="nttCmtCn" class="none">Ingreso de la respuesta</label>
			<input type="image" src="/resources/web/theme/spanish/images/btn/btn_reply.gif" alt="Ingreso de la respuesta" onclick="jsReplyInsert('commentModifyForm'); return false;">
<!-- 			<img title="button" src="/resources/web/theme/default/images/btn/btn_comment.gif" onclick="jsReplyInsert('commentModifyForm');" alt="등록" /> -->
    ...
    <div id="activeCommentReply" class="none">
        ...
        <div class="textarea">
	        ...
	        <label for="nttCmtCn" class="none">Ingreso de la respuesta</label>
            <input type="image" src="/resources/web/theme/spanish/images/btn/btn_reply.gif" alt="Ingreso de la respuesta"  onclick="jsReplyInsert('commentReplyForm');  return false;">
        </div>
    </div>
    <!-- //댓글의 답글 등록 -->
	...
	<div class="reply_box">
	   ...
	   <span class="totalcount"> Total <em class="tx_orange">${pagerVo.totalNum}</em></span>
						...
						<c:forEach items="${pagerVo.list}" var="commentVo" varStatus="status" >
							<li id="reply${status.index+1}" class="${commentVo.cmtDp ne 0? 'reply_list':''}" ><!-- reply${commentVo.cmtDp+1} 를 지웠음 ㅠㅠ ★★★-->
								<div class="cont_l">
									...
									<span class="date"> ${commentVo.registDt} <c:if test="${!empty commentVo.updtDt}">(Edición final  : <c:out value='${commentVo.updtDt}'/>)</c:if></span>
									<c:if test="${commentVo.cmtDp lt 1 and boardConfVo.bbsCd  ne 1010 and commentVo.mngrDeleteAt ne 'Y'}">
										<c:if test="${commentVo.userDeleteAt ne 'Y' }">
											<div id="btnReply${status.index+1}" class="btn_reply block">
											  <a href="#" onclick="jsCommentReplyForm('<c:out value='${commentVo.domainCd}'/>','<c:out value='${commentVo.bbsCd}'/>', '<c:out value='${commentVo.bbscttSeq}'/>', '<c:out value='${commentVo.cmtSeq}'/>', '<c:out value='${commentVo.mngrId}'/>', '<c:out value='${commentVo.registId}'/>','<c:out value='${status.index+1}'/>');" title="Respuesta">Respuesta</a>
											</div>
										</c:if>
									</c:if>
									<div id="btnReplyCancle${status.index+1}" class="btn_reply_cancle none">
                                        <a href="#" onclick="jsReplyFormRemove();" title="Respuesta Cancelar">Respuesta Cancelar</a>
									</div>
									<div id="btnModifyCancle${status.index+1}" class="btn_update_cancle none">
									  <a href="#" onclick="jsModifyFormRemove();" title="Editar Cancelar">Editar Cancelar</a>
									</div>
								</div>
										...
										<c:if test="${!empty __usk.userKey && __usk.userKey eq commentVo.userKey}">
											<c:if test="${commentVo.userDeleteAt ne 'Y'}"> 
												<c:if test="${boardConfVo.bbsCd  ne 1010 }"> 
												     <li><a href="#" onclick="jsCommentModify('<c:out value='${commentVo.domainCd}'/>','<c:out value='${commentVo.bbsCd}'/>', '<c:out value='${commentVo.bbscttSeq}'/>', '<c:out value='${commentVo.cmtSeq}'/>', '<c:out value='${commentVo.registId}'/>','<c:out value='${status.index+1}'/>');" title="Editar">Editar</a></li>
												</c:if>
											         <li><a href="#" onclick="jsDeleteComment('<c:out value='${commentVo.domainCd}'/>','<c:out value='${commentVo.bbsCd}'/>', '<c:out value='${commentVo.bbscttSeq}'/>', '<c:out value='${commentVo.cmtSeq}'/>', '<c:out value='${commentVo.registId}'/>','<c:out value='${status.index+1}'/>');" title="Eliminar">Eliminar</a></li>
										    </c:if>
									    </c:if>
									</ul>
								</div>
							...
							<li class="comment" id="comments${status.index+1}">
								<c:choose>
									<c:when test="${commentVo.userDeleteAt eq 'Y' }">
										<c:if test="${commentVo.cmtDp != 0 }"><div class="reply">* Respuesta eliminada por el usuario.</div></c:if>
										<c:if test="${commentVo.cmtDp == 0 }">* Respuesta eliminada por el usuario.</c:if>
									</c:when>
									<c:when test="${commentVo.mngrDeleteAt eq 'Y' }">
										<c:if test="${commentVo.cmtDp != 0 }"><div class="reply">* Respuesta eliminada por el administrador</div></c:if>
										<c:if test="${commentVo.cmtDp == 0 }">* Respuesta eliminada por el administrador</c:if>
									</c:when>
									<c:otherwise>
		  ...
		  <!-- //의견글내용-->
		    ...
		    <form id="commentForm" action="ND_cmt.insert.do" method="post" onSubmit="return false">
				...
				<div class="textarea">
					<textarea name="nttCmtCn" id="nttCmtCn" rows="3" cols="30"></textarea>
					<label for="nttCmtCn" class="none">Ingreso de la respuesta</label>
                            <input type="image" src="/resources/web/theme/spanish/images/btn/btn_reply.gif" alt="Ingreso de la respuesta" onclick="jsReplyInsert('commentForm');  return false;">
