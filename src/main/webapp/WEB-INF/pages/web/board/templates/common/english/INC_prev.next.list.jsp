...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
    ...
    <table class="tbl_prevNext">
        ...
        <tbody>
            <tr>
                <td><span class="prev">Previous post</span></td>
                <!-- 이전글 -->
                <c:if test="${empty dataVo.prevVO}"><td class="t_al" colspan="${boardConfVo.bbsCd eq '1003'? '4':'3'}">No previous posting</td></c:if>
                <c:if test="${!empty dataVo.prevVO}">
                    <c:choose>
                        <c:when test="${dataVo.prevVO.mngrDeleteAt eq 'Y'}">
                          <td class="t_al">[Deleted by administrator.]</td>
                        </c:when>
                        <c:otherwise>
                            <td class="t_al">
...
<!--                                      <img src="/resources/web/theme/default/images/icon/icon_new.gif" alt="새글"/> -->
                            ...
                            <c:if test="${boardConfVo.bbsCd eq '1003'}">
                                <td class="${dataVo.prevVO.estnColumn3 eq 'N' ? 'fc_brown':'' }">
                                    <c:if test="${dataVo.prevVO.estnColumn3 eq 'Y'}">Response completed</c:if>
                                    <c:if test="${dataVo.prevVO.estnColumn3 eq 'N'}">Not responded</c:if>
                                </td>
                            </c:if>
            ...
            <tr>
                <td><span class="next">Next post</span></td>
                <c:if test="${empty dataVo.nextVO}"> <td class="t_al" colspan="${boardConfVo.bbsCd eq '1003'? '4':'3'}">No following posting.</c:if>
...
<!--                                      <img src="/resources/web/theme/default/images/icon/icon_new.gif" alt="새글"/> -->
                            ...
                            <c:if test="${boardConfVo.bbsCd eq '1003'}">
                                <td class="${dataVo.nextVO.estnColumn3 eq 'N' ? 'fc_brown':'' }">
                                    <c:if test="${dataVo.nextVO.estnColumn3 eq 'Y'}">Response completed</c:if>
                                    <c:if test="${dataVo.nextVO.estnColumn3 eq 'N'}">Not responded</c:if>
                                </td>
                            </c:if>
