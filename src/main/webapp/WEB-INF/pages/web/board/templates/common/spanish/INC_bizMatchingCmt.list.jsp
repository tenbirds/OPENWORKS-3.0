    ...
    $().ready(function() {
        ...
        $("#commentForm").validate({
            ...
            messages : {
                ...
                bizMatchingCttpc1 : {
                    required : "Ingrese la información de contacto.",
                },
                bizMatchingCttpc2 : {
                    required : "Ingrese la información de contacto.",
                },
    ...
    var jsSelection = function(domainCd, bbsCd, bbscttSeq, cmtSeq) {
        ...
        $.ajax({
            ...
            success : function(result) {
				...
				var html =""; 
				html += '<div class="btn_b_round"><span>Selección Completo</span></div>';
				$("#"+cmtSeq).append(html);
...
<c:if test="${cmtBoardVo.bizProgression eq 'PRO'}">
	<form id="commentForm" name="commentForm" action="ND_cmt.insert.do"method="post" enctype="multipart/form-data" onsubmit="return false;">
		...
		<table class="tbl_view borb_n">
			...
			<tbody>
				<tr>
					<td class="tit"><label for="bizMatchingCmpnyNm">Nombre de la empresa</label></td>
                    <c:choose>
                        <c:when test="${dataVo2.userTyCd eq '1001' || dataVo2.userTyCd eq '1002'}">
                            <td><input type="text" id="bizMatchingCmpnyNm" name="bizMatchingCmpnyNm" title="Ingrese Nombre de la empresa" value="${dataVo2.indvdlCmpnyNm}" class="input" style="width: 170px;"/></td>
                        </c:when>
                        <c:otherwise>
                            <td><input type="text" id="bizMatchingCmpnyNm" name="bizMatchingCmpnyNm" title="Ingrese Nombre de la empresa" value="${dataVo2.entrprsCmpnyNm}" class="input" style="width: 170px;"/></td>
                        </c:otherwise>
                    </c:choose>
					<td class="tit"><label for="bizMatchingCttpc">Contacto</label></td>
                    <c:choose>
                        <c:when test="${dataVo2.userTyCd eq '1001' || dataVo2.userTyCd eq '1002'}">
                            <td>
                                <input type="text" id="bizMatchingCttpc1" name="bizMatchingCttpc1" title="Ingrese Contacto" value="${fn:split(dataVo2.indvdlCmpnyReprsntTelno,'-')[0]}" class="input" style="width: 55px;" maxlength="4"/> - 
                                <input type="text" id="bizMatchingCttpc2" name="bizMatchingCttpc2" title="Ingrese Contacto" value="${fn:split(dataVo2.indvdlCmpnyReprsntTelno,'-')[1]}" class="input" style="width: 55px;" maxlength="4"/> - 
                                <input type="text" id="bizMatchingCttpc3" name="bizMatchingCttpc3" title="Ingrese Contacto" value="${fn:split(dataVo2.indvdlCmpnyReprsntTelno,'-')[2]}" class="input" style="width: 55px;" maxlength="4"/>
                            </td>
                        </c:when>
                        <c:otherwise>
                            <td>
                                <input type="text" id="bizMatchingCttpc1" name="bizMatchingCttpc1" title="Ingrese Contacto" value="${fn:split(dataVo2.entrprsCmpnyReprsntTelno,'-')[0]}" class="input" style="width: 55px;" maxlength="4"/> - 
                                <input type="text" id="bizMatchingCttpc2" name="bizMatchingCttpc2" title="Ingrese Contacto" value="${fn:split(dataVo2.entrprsCmpnyReprsntTelno,'-')[1]}" class="input" style="width: 55px;" maxlength="4"/> - 
                                <input type="text" id="bizMatchingCttpc3" name="bizMatchingCttpc3" title="Ingrese Contacto" value="${fn:split(dataVo2.entrprsCmpnyReprsntTelno,'-')[2]}" class="input" style="width: 55px;" maxlength="4"/>
                            </td>
                        </c:otherwise>
                    </c:choose>
					<td class="tit"><label for="bizMatchingEmail">Correo electrónico</label></td>
                    <c:choose>
                        <c:when test="${dataVo2.userTyCd eq '1001' || dataVo2.userTyCd eq '1002'}">
                            <td class="last"><input type="text" id="bizMatchingEmail" name="bizMatchingEmail" title="Ingrese Correo electrónico" value="${dataVo2.indvdlCmpnyEmail}" class="input" style="width: 170px;"/></td>
                        </c:when>
                        <c:otherwise>
                            <td class="last"><input type="text" id="bizMatchingEmail" name="bizMatchingEmail" title="Ingrese Correo electrónico" value="${dataVo2.entrprsCmpnyEmail}" class="input" style="width: 170px;"/></td>
                        </c:otherwise>
                    </c:choose>
				</tr>
				<tr>
                    <td class="tit"><label for="multiFiles">Contenido</label></td>
                    <td colspan="5" class="last">
                        <div class="cont_wrap">
                            <%--<textarea id="nttCmtCn" name="nttCmtCn" cols="72" rows="50" style="width: 877px; height: 200px;" title="Ingrese Contenido" placeholder="Por favor, tenga en cuenta que la sesión se desconectará al escribir el contenido durante mucho tiempo."></textarea>--%>
                            <textarea id="nttCmtCn" name="nttCmtCn" cols="72" rows="50" style="width: 877px; height: 200px;" title="Ingrese Contenido"></textarea>
                            <!--
                            <textarea id="nttCmtCn" name="nttCmtCn" cols="72" rows="50" style="width: 877px; height: 62px;" title="Ingrese Contenido"></textarea>
                            <input type="button" onclick="jsSubmit(); return false;" title="Registrarse" class="btn_register" style="margin-left: 10px;"/>
                            -->
                        </div>
                        <!--
                        <div class="check">
                            <input type="checkbox" id="bizMatchingOthbcAt" name="bizMatchingOthbcAt" class="rach" value="Y"/>
                            <label for="bizMatchingOthbcAt">Divulgación</label>
                        </div>
                         -->
                    </td>
				</tr>
                ...
                <tr>
                    ...
                    <td colspan="4" class="last">
                        ...
                        <div class="guide_txt" style="padding-top: 5px;">
                            ...
                            ※ Se pueden cargar archivos de hasta ${boardConfVo.maxFileSize}
                            <c:if test="${fn:length(boardConfVo.filePermExtsn) >= 3 }">
                                ...
                                en formato ${join_filePermExtsn}
                            </c:if>
                            <!-- <br /><br/><b>※ 비즈매칭 선정시 연락처와 이메일이 비즈매칭 요청자에게 공개됩니다.</b> -->
                        </div>
                    </td>
                    <td class="last">
                        <div class="cont_wrap">
				...
				<tr>
					<c:choose>
						<c:when test="${commentVo.mngrDeleteAt eq 'Y'}">
							<!-- 관리자 삭제댓글 -->
							<td class="txt" colspan="2">Reply deleted by administrator</td>
							<td></td>
						</c:when>
						<c:when test="${commentVo.bizMatchingOthbcAt eq 'N' and (!empty __usk.userKey && (__usk.userKey eq commentVo.userKey or __usk.userKey eq cmtBoardVo.userKey ))}">
							...
							<td class="txt">
                                ...
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <div class="add_file">No hay archivos adjuntos.</div>
                                    </c:otherwise>
                                </c:choose>
									...
									<c:if test="${empty cmtBoardVo.bizCmtSeq and commentVo.mngrDeleteAt != 'Y'}">
										<c:if test="${!empty __usk.userKey && __usk.userKey eq commentVo.userKey}">
											<c:if test="${empty cmtBoardVo.bizCmtSeq and cmtBoardVo.bizCmtSeq ne commentVo.cmtSeq}">
												<span><a href="#"onclick="jsModifyCommentAction('<c:out value='${commentVo.domainCd}'/>','<c:out value='${commentVo.bbsCd}'/>', '<c:out value='${commentVo.bbscttSeq}'/>', '<c:out value='${commentVo.cmtSeq}'/>', '<c:out value='${commentVo.mngrId}'/>', '<c:out value='${commentVo.registId}'/>');">Editar</a></span>
												<span><a href="#"onclick="jsDeleteCommentAction('<c:out value='${commentVo.domainCd}'/>','<c:out value='${commentVo.bbsCd}'/>', '<c:out value='${commentVo.bbscttSeq}'/>', '<c:out value='${commentVo.cmtSeq}'/>', '<c:out value='${commentVo.mngrId}'/>', '<c:out value='${commentVo.registId}'/>');">Eliminar</a></span>
											</c:if>
										</c:if>
									</c:if>
								</div>
								...
								<ul>
                                    <c:choose>
                                        <c:when test="${(!empty __usk.userKey && __usk.userKey eq commentVo.userKey) or (!empty cmtBoardVo.bizCmtSeq and cmtBoardVo.bizCmtSeq eq commentVo.cmtSeq)}">
                                            <!-- 비공개인경우(댓글작성자 또는 글등록자 인경우)에서 댓글작성자 또는 선정인경우 연락처 공개-->
                                            <li class="company"><span class="tit">Nombre de la empresa</span><c:out value='${commentVo.bizMatchingCmpnyNm}'/></li>
                                            <li class="address"><span class="tit">Contacto</span><c:out value='${commentVo.bizMatchingCttpc}'/></li>
                                            <li class="id"><span class="tit">Correo electrónico</span><c:out value='${commentVo.bizMatchingEmail}'/></li>
                                        </c:when>
                                        <c:otherwise>
                                            <c:set var="Cttpc" value="${fn:length(commentVo.bizMatchingCttpc)-4}" />
                                            <c:set var="Email" value="${fn:length(commentVo.bizMatchingEmail)-3}" />
                                            <li class="company"><span class="tit">Nombre de la empresa</span><c:out value='${commentVo.bizMatchingCmpnyNm}'/></li>
                                            <li class="address"><span class="tit">Contacto</span><c:out value='${fn:substring(commentVo.bizMatchingCttpc,0,Cttpc)}'/>****</li>
                                            <li class="id"><span class="tit">Correo electrónico</span><c:out value='${commentVo.maskBizMatchingEmail}'/></li>
                                        </c:otherwise>
                                    </c:choose>
                                </ul>
							</td>
							<td id="${commentVo.cmtSeq}" ><c:if test="${!empty __usk.userKey && __usk.userKey eq cmtBoardVo.userKey}">
									...
									<c:if test="${!empty cmtBoardVo.bizCmtSeq and cmtBoardVo.bizCmtSeq eq commentVo.cmtSeq}">
										<div class="btn_b_round"><span>Selección${empty cmtBoardVo.bizCmtSeq? '' :'Completo'}</span></div>
									</c:if>
								</c:if>
						  </td>
						</c:when>
						<c:when
							...
							<td class="txt" colspan="3">
								<div class="con">
									<strong>Este es un mensaje oculto. </strong>
								</div>
							</td>
							<td></td>
						</c:when>
...
<c:if test="${commentVo.mngrDeleteAt eq 'Y'}">Reply deleted by administrator</c:if>
