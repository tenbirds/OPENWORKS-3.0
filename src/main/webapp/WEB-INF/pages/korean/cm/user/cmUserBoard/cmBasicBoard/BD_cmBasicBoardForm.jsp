...
<c:if test="${userBoardView.cmmntyMenuCode == '1001' }">
	<c:if test="${cmInfo.userId != sessionScope['__usk'].userId}">
	<script  type="text/javascript">
		alert("<c:out value="${cmUserMenuView.cmmntyMenuNm}"/>은/는 "+Message.msg.chkCmOptFormOk); // 공지사항은 운영자만 작성 
		// 이전소스
		//location.href="http://<c:out value='${SubDomain}'/>.ceart.kr/<c:out value='${SubDomain}'/>/cm/user/cmUserMain/BD_index.do?cmmntyMenuCode=0&cmmntyId=<c:out value="${cmUserMenuView.cmmntyId}"/>";
    ...
    var jsCmBoardUpdate = function(){
	    ...
	    if ( confirm(Message.msg.chkModify) ) {
 	        $("#dataForm").ajaxSubmit({
   	            url      : "INC_cmBasicBoardUpdate.do",
   	            type     : "POST",
   	            dataType : "text",
   	            success  : function(response) {
   	                if (response == "true") {
   	                 	alert(Message.msg.chkCmBoardUpdateOk);  
	                    jrMove("BD_cmBasicBoardView.do?cmmntyMenuCode=<c:out value='${cmUserMenuView.cmmntyMenuCode}'/>&cmmntyId=<c:out value='${cmUserMenuView.cmmntyId}'/>");
	               } else { 
	                   alert(Message.msg.processFail);
	                   return;
	               }
...
// 		    jsPreview('file_add',inputNm, 0, 0, 5120,"exe,jsp,zip,class,alz");
		    ...
		    /* 20150827 제한용량 : 10240 -> 51200, 확장자 추가 : jpg,jpeg,png,gif,hwp,doc,docx -> */
		    jsPreview('pfile1', inputNm , 3000, 3000, 51200, 'jpg,jpeg,png,gif,hwp,doc,docx,xls,xlsx,ppt,pptx,pdf,zip,rar,gz,tar');
		};
		//서비스이미지 용량,크기,비율 확인
	...
	<input type="hidden" id="atchFileSeq" name="atchFileSeq" value="<c:out value="${userBoardView.atchFileSeq }"/>"/>
		...
		<div id="container">
			...
			<div id="contents">
				...
				<table class="tbl_view">
					...
					<tbody>
						<tr>
							<td class="tit">제목</td>
							<td class="last">
								<input type="text" id="bbscttSj" name="bbscttSj" title="제목 입력" value="<c:out value="${userBoardView.bbscttSj }"/>" class="input" style="width:515px;" maxlength="100" >
								<c:if test="${cmInfo.userId == sessionScope['__usk'].userId}">
						...
						<tr>
							<td colspan="2" class="last">
							   <div id="contentsErrorDiv"></div>
			                   	...
			                   	<valid:msg name="bbscttCn"></valid:msg>
                   			</td>
						</tr>
						</c:if>
						<c:if test="${cmInfo.userId != sessionScope['__usk'].userId}">
						<tr>
							<td colspan="2" class="last">
							<textarea id="bbscttCn" name="bbscttCn" style="width:725px; height:360px;" onkeyup="javascript:displayBytes(1000,'bbscttCn');"  onblur="javascript:displayBytes(1000,'bbscttCn');" ><c:out value="${userBoardView.bbscttCn }" escapeXml="false"/></textarea>
							</td>
						</tr>
						</c:if>
						<tr>
							...
							<c:if test="${empty userBoardView.cmUserBoardFileCollection}">
								...
								<div class="guide_txt">※ 지원파일 : jpg, jpeg, png, gif, hwp, doc, docx, xls, xlsx, ppt, pptx, pdf, zip, rar, gz,tar</div>
								<div class="guide_txt">※ 지원용량 : 50MB 미만 파일만 업로드 가능합니다.</div>
							</td>
						</tr>
						<tr>
							<td class="tit">태그</td>
							<td class="last">
							<input type="text" id="tagNm" name="tagNm" title="태그 입력" value="" class="input" style="width:570px;">
						...
						<tr>
							...
							<td class="last">
								...
								<input type="radio" id="othbcSetupCd2" name="othbcSetupCd" value="1004" <c:if test="${userBoardView.othbcSetupCd == '1004' }"> checked="checked" </c:if>/><label for="member">비밀글</label>
							</td>
						</tr>
					</tbody>
				</table>
				...
				<div class="btn_area">
					<div class="btn_l">
						<div class="btn_navi"><a href="#none" onclick="jrMove('BD_cmBasicBoardList.do?cmmntyId=<c:out value="${cmUserMenuView.cmmntyId}"/>')"><span class="list">목록</span></a></div>
					</div>
					<div class="btn_r">
