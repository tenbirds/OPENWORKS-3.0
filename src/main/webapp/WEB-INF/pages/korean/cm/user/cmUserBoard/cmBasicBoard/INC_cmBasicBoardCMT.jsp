...
<script type="text/javascript">
 ...
 // 페이지 이동
    ...
    // 댓글 폼에 데이터 세팅
    var jsCommentModify= function(form, cmmntyBbsSeq, cmmntyCmtSeq, refrnCmtSeq, cmtDp, secret){
        ...
        $.post("INC_cmBasicBoardCMTForm.do", {
            cmmntyId : cmmntyId,
        ...
        function(result) {
            if(result.result == true) {
                ...
                if(cmtDp == "0"){
                   ...
                   if(secret == "Y")
                       $("#"+form+" #secretYn").attr("checked",true);
                   else 	
                       $("#"+form+" #secretYn").attr("checked",false);
                	...
                	jsModifyFormRemove($("#dataForm #formSeq").val());
              		...
              		$("#replyHtml"+cmmntyCmtSeq).find('input[name=cmtDp]').val(result.value.cmtDp);
               	    ...
               	    $("#replyHtml"+cmmntyCmtSeq + " #secretYnArea").hide();
 ...
 // 답글 입력/수정
    var jsReplyUptInsert = function(form) {
        ...
        if(form == "commentReplyForm") {
           cmtUrl = "INC_cmBasicBoardCMTInsert.do?bbsCmt=reply";
           $("#commentForm *").attr('disabled', true);
        } else if(form == "commentModifyForm"){
            cmtUrl = "INC_cmBasicBoardCMTUpdate.do?bbsCmt=reply";
            $("#commentForm *").attr('disabled', true);
        }
    ...
    var jsCommentUptInsert = function(form , type) {
        ...
        var cmmntyBbsSeq = $("#commentForm #cmmntyBbsSeq").val();
	        if(type == "update"){
	            cmtUrl =  "INC_cmBasicBoardCMTUpdate.do?bbsCmt=comment"; 
	        }else{
	            cmtUrl =  "INC_cmBasicBoardCMTInsert.do?bbsCmt=comment";
	        }
 	...
 	var jsDeleteComment= function(form, cmmntyBbsSeq, cmmntyCmtSeq, refrnCmtSeq, cmtDp ){
	     ...
	     if(confirm(Message.msg.chkCmBoardCmt)){
		       $.ajax({
		         url      : "INC_cmBasicBoardCMTDelete.do",
		         type     : "POST",
     	...
     	return false;
					...
					<!-- 답글 수정  -->
						<div id="replyCommentUpdateDiv" style="display: none;">
					        ...
					        <span id="secretYnArea" style="margin-top:10px; text-align:right; display:block;"><input type="checkbox" id="secretYn"  name="secretYn" value='Y'/> <label for="">비밀댓글</label></span>
							<div class="textarea">
					...
					<div class="comment_area">
						...
						<div class="comment_list" style="display: block;">
							<ul>
								...
								<c:if test="${ data.secretYn == 'Y' and sessionScope['__usk'].userId != data.registId and cmInfo.userId != sessionScope['__usk'].userId }" >
								<li class="comment" >비밀글입니다 .</li>
								<c:set var="commen_secret_chk" value="Y"/>
								</c:if>
								<c:if test="${ data.secretYn == null || (data.secretYn == 'Y' and (sessionScope['__usk'].userId == data.registId || cmInfo.userId == sessionScope['__usk'].userId  )) }" >
								<c:set var="commen_secret_chk" value="N"/>
								<li>
									<div class="cont_l" >
										<c:if test="${ cmInfo.userId != data.registId}">
											<strong> <c:if test="${ data.secretYn == 'Y'}">[비밀댓글]</c:if> <c:out value='${data.wrterNcnm}'/></strong>
										</c:if>
										<c:if test="${cmInfo.userId == data.registId }">
											<strong> <c:if test="${ data.secretYn == 'Y'}">[비밀댓글]</c:if> 운영자2</strong>
										</c:if>
									...
									<div class="cont_r">
										<ul>
											<li><a href="#none" onclick="jsCommentModify('commentForm','<c:out value='${ data.cmmntyBbsSeq}'/>','<c:out value='${ data.cmmntyCmtSeq}'/>', '<c:out value='${ data.refrnCmtSeq}'/>', '<c:out value='${ data.cmtDp}'/>' , '<c:out value='${ data.secretYn}'/>')">수정</a></li>
								...
								<li class="reply_list">
									...
									<div class="cont_r">
										<ul>
											<li><a href="#none" onclick="jsCommentModify('commentModifyForm','<c:out value='${ data.cmmntyBbsSeq}'/>','<c:out value='${ data.cmmntyCmtSeq}'/>', '<c:out value='${ data.refrnCmtSeq}'/>', '<c:out value='${ data.cmtDp}'/>', '<c:out value='${ data.secretYn}'/>')">수정</a></li>
											<li><a href="#none" onclick="jsDeleteComment('commentForm','<c:out value='${ data.cmmntyBbsSeq}'/>','<c:out value='${ data.cmmntyCmtSeq}'/>', '<c:out value='${ data.refrnCmtSeq}'/>', '<c:out value='${ data.cmtDp}'/>' )">삭제</a></li>
										</ul>
									</div>
								</li>	
								...
								<!-- 답글 입력 -->
								<li  class="comment"  id="replyHtml<c:out value='${ data.cmmntyCmtSeq}'/>" class="reply" style="display:none;"></li>
								<!-- //답글 입력 -->
								</c:forEach>
							</ul>
						</div>
...
<%-- 		            cmmntyMenuCode:"<c:out value="${cmUserMenuView.cmmntyMenuCode}"/>", cmmntyBbsSeq:"<c:out value="${userBoardView.cmmntyBbsSeq}"/>"},  --%>
						    <form id="commentForm" action="INC_cmBasicBoardCMTInsert.do" name="commentForm" method="post"  onSubmit="return false">
						        <input type="hidden" name="cmmntyId" id="cmmntyId" value="<c:out value='${param.cmmntyId}'/>" />
						        ...
						        <span style="margin-top:10px; text-align:right; display:block;"><input type="checkbox" id="secretYn" name="secretYn"  value='Y'/> <label for="">비밀댓글</label></span>
								<div class="textarea">
									<textarea id="cmtCn" name="cmtCn" onkeyup="javascript:displayBytes(500,'cmtCn');"></textarea>
