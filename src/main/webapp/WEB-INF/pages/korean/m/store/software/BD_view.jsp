	...
	<div class="conWrap new_swView" id="wrap_ver2">
		<div class="pro_title">
	        <div class="f_l">
				...
				<c:if test="${dataVo.goodsPc ne null and dataVo.goodsPc ne '' }">
				<p class="pro_price">\ ${dataVo.goodsPc}
					<%-- <span>계약단위 : <c:forEach items="${dataVo.goodsMetaCntrctUnit}" var="_goodsMetaCntrctUnit" varStatus="status">	<c:if test="${!status.first}">, </c:if>${_goodsMetaCntrctUnit.INDVDLZ_CD_NM}<c:if test="${_goodsMetaCntrctUnit.INDVDLZ_CD eq 1005}">(${_goodsMetaCntrctUnit.META_ETC_CN})</c:if></c:forEach></span> --%>
				</p>
				</c:if>
				<div>
					<c:if test="${not empty dataVo.goodsDwldUrl}">
					<a href="${dataVo.goodsDwldUrl}" target="_blank"  class="btns st1">서비스 안내</a>
					</c:if>
<%--					
...
<%-- 			
		<ul class="sns_area">
			<li><a href="javascript:fn_sendSns('facebook','${dataVo.goodsNm}');">페이스북</a></li>
			<li><a href="javascript:fn_sendSns('twitter','${dataVo.goodsNm}');">트위터</a></li>
			<li><a href="javascript:fn_sendSns('naver','${dataVo.goodsNm}');">블로그</a></li>
			<li><a href="javascript:;" onclick="jsItst('${dataVo.goodsCode}');" class="favor <c:if test="${intrstcnt > 0}">on</c:if>">즐겨찾기</a></li>
			<li><a href="javascript:;" onClick="pgPrint();return false;">프린트</a></li>
		</ul>
...
--%>		
		<section>
			<c:if test="${not empty ppsList}">
			<h3><i>나라장터 종합쇼핑몰</i></h3>
			<div>
				<table border="0" cellspacing="0" cellpadding="0"class="basic_table" width="100%" class="basic_table">
					<caption></caption>
					<colgroup>
						<col width="48px">
						<col width="*">
					...
					<tr>
						...
						<td><input  name="basktGoodsQy"  type="text" placeholder="1"></td>
			...
			<div>
				<table border="0" cellspacing="0" cellpadding="0" class="normal_table" width="100%">
					...
					<tr>
						<th>서비스 관리기준</th>
						<td><c:forEach items="${dataVo.goodsMetaSvc}" var="_goodsMetaSvc" varStatus="status"><c:if test="${!status.first}">, </c:if>${_goodsMetaSvc.INDVDLZ_CD_NM}</c:forEach></td>
					</tr>
					<!-- <tr>
						<th>API access</th>
						<td>해당사항 없음 - 데이터 확인 </td>
					</tr> -->
					<tr>
						<th>인증정보</th>
						<td>
							<c:forEach items="${dataVo.goodsMarkList}" var="_goodsMark" varStatus="status">
								<c:forEach items="${dataVo.markFile[status.index]}" var="fileVo" varStatus="status1">
									<img src="${fileVo.fileUrl}" title="${_goodsMark.markNm}" width="33" height="33"/> ${_goodsMark.markNm}
					...
					<tr>
						<th>아키텍처</th>
						<td><c:if test="${not empty dataVo.archtcSe}">
								<c:choose>
					...
					<tr>
						<th>지원OS</th>
						<td><c:forEach items="${dataVo.goodsMetaOS}" var="_goodsMetaOS" varStatus="status">
					...
					<tr>
						<th>지원언어</th>
						<td><c:forEach items="${dataVo.goodsMetaLang}" var="_goodsMetaLang" varStatus="status"><c:if test="${!status.first}">, </c:if>${_goodsMetaLang.INDVDLZ_CD_NM}</c:forEach></td>
					</tr> 
					<tr>
						<th>A/S</th>
						<td><c:forEach items="${dataVo.goodsMetaAS}" var="_goodsMetaAS" varStatus="status">
					...
					<tr>
						<th>호환성</th>
						<td><c:forEach items="${dataVo.goodsMetaStd}" var="_goodsMetaStd" varStatus="status"><c:if test="${!status.first}">, </c:if>${_goodsMetaStd.INDVDLZ_CD_NM}</c:forEach></td>
					</tr>
					<tr>
						<th>제조사</th>
...
<%-- 					
 ...
 --%>					
			...
			<c:if test="${ not empty dataVo.goodsDwldUrl and not empty dataVo.goodsDwldUrl and not empty dataVo.etcFile}">
			...
			<div>
				<table border="0" cellspacing="0" cellpadding="0" class="normal_table" width="100%">
					...
					<tr>
						<th>서비스 안내</th>
						<td>${dataVo.goodsDwldUrl}<a href="${dataVo.goodsDwldUrl}" target="_blank" class="btn_ml"><img src="/cloud_ver2/new_cloud/images/icn_txt_link.png" alt="바로가기"></a></td>
					</tr>
					</c:if>
					 --%>
					...
					<tr>
						<th class="font_normal font14 borB0">기타 첨부파일</th>
...
<%-- 			
			...
			<div class="ps_r">
				<div id="firstImg">
					...
					<img src="${dataVo.imageFile[1].fileUrl}" width="260" height="317">
					</c:if>
				</div>
						...
						<c:if test="${ not empty dataVo.imageFile[1].fileUrl }">
				            ...
				            <c:if test="${(j-1)*5 < (fn:length(dataVo.imageFile)-1)}">
								...
								<c:if test="${!empty dataVo.imageFile[i]}">
				                 	<li class="small_screen_img"><!-- <a href="javascript:;" onclick="jsTab(this, 'screen');"> --><span id="imgFiles" style="cursor: pointer;" onclick="fnLoadImg('${dataVo.imageFile[i].fileUrl}')"><img src="${dataVo.imageFile[i].fileUrl}" alt="${dataVo.goodsNm}"  /></span><img src="${dataVo.imageFile[i].fileUrl}" alt="${dataVo.goodsNm}"  /><!-- </a> --></li>
				              	</c:if>
				              	</c:forEach>
				            </c:if>
				            </c:forEach>
 ...
 <%-- 
			<h3><i>Q&amp;A</i></h3>
			<iframe title="상품문의하기" id="inquiryFrame" src="../inc/INC_inquiry.do?goodsCode=${dataVo.goodsCode}&pageUrlNm=${dataVo.pageUrlNm}"
              width="100%" frameborder=0 framespacing=0 marginheight=0 marginwidth=0 scrolling=no vspace=0 ></iframe>
