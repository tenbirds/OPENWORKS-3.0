...
<table class="gray_list" cellspacing="0" border="0" summary="기본게시판 목록으로 번호,분류,제목,작성자,작성일 등의 조회 정보를 제공합니다.">
	...
	<thead>
		<tr>
			...
			<!-- 상태(필수) -->
			<th id="th-ESTN_COLUMN1" class="lr_none"><span class="sort cs_pointer" id="ESTN_COLUMN1">상태</span></th>
		</tr>
	</thead>
	<tbody id="odd-color">
		<!-- 공지 목록  -->
							...
							<c:when test="${arrange.columnId eq 'BBSCTT_SJ'}">
								<td class="tx_l">
									<a href="#" rel="#titleNoticeSummary${status.count}" class="contentTip"
										onclick="jsView('${noticeVo.bbsCd}', '${noticeVo.bbscttSeq}', '${noticeVo.registPassword}', '${noticeVo.othbcAt}'); return false;">
										${noticeVo.bbscttSj}
		...
		<c:forEach items="${pager.list}" var="baseVo" varStatus="status">
			<tr>
								...
								<c:choose>
									...
									<c:otherwise>
										<a href="#" rel="#titleSummary${status.count}" class="contentTip"
											onclick="jsView('${baseVo.bbsCd}', '${baseVo.bbscttSeq}', '${baseVo.registPassword}', '${baseVo.othbcAt}'); return false;">
											${baseVo.bbscttSj}
				...
				<td class="lr_none">
					<c:choose>
						<c:when test="${baseVo.estnColumn1 eq 'A'}">접수중</c:when>
						<c:when test="${baseVo.estnColumn1 eq 'B'}">접수완료</c:when>
						<c:when test="${baseVo.estnColumn1 eq 'C'}">처리중</c:when>
						<c:when test="${baseVo.estnColumn1 eq 'D'}">처리완료</c:when>
