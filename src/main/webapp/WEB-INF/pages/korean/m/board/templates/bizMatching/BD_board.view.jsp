...
<body>
		...
		<div id="container" class="join-wrap">
			...
			<div class="contents">
						...
						<div class="type-write">
							<table>
								...
								<tbody>
									...
									<%-- <tr><td colspan="8">'${arrange.columnId}'</td></tr> --%>
										<c:choose>
														...
														<td colspan="7">
															<c:if test="${not empty dataVo.ctgryList}">
																<c:forEach items="${dataVo.ctgryList}" var="ctgryVo">
							                                    	<p class="cate"><span>[${ctgryVo.ctgryClNm }]${ctgryVo.ctgryPath }</span></p>
							                                    </c:forEach>
						                                    </c:if>
						                                    <c:if test="${empty dataVo.ctgryList}"><p class="cate"><span>등록하신 카테고리를 찾을수없습니다.</span></p></c:if>
														</td>
													</tr>
													<tr>
														<th><span><label for="${arrange.beanNm}">${arrange.columnNm}</label></span></th>
														<td colspan="3">${dataVo.bbscttSj}</td>
														<th><span>매칭수</span></th>
											...
											<!-- 내용 -->
												<tr>
													<th><span><label for="${arrange.beanNm}">${arrange.columnNm}</label></span></th>
													<td colspan="7"><div class="txt-cont"><op:bean-util field="${arrange.columnId}" obj="${dataVo}"/></div></td>
												</tr>
											</c:when>
														...
														<c:choose>
															<c:when test="${fn:length(dataVo.fileList) > 0}">
																<c:forEach items="${dataVo.fileList}" var="fileVo">
																	...
																	<img src="/new_cloud/images/icon-btn/icon-file-add.gif" alt="" />${fileVo.localNm} (${fileVo.fileSize})
																	</a>
																	</p>
																</c:forEach>
															</c:when>
															<c:otherwise>
																<p class="file-add ar">첨부파일이 없습니다.</p>
															</c:otherwise>
														</c:choose>
														</td>
													</tr>
				...
				<div class="btn-area">
					<p class="fl">
						<a href="#" class="b-btn list type1" onclick="jsList('${param.q_currPage}');"><strong><span>목 록</span></strong></a>
						<c:if test="${boardConfVo.listViewCd eq 1001}">
							<a href="#" class="b-btn type1" onclick="jsView('${dataVo.prevVO.domainCd}','${dataVo.prevVO.bbsCd}', '${dataVo.prevVO.bbscttSeq}', '${dataVo.prevVO.registPassword}', '${dataVo.prevVO.othbcAt}'); return false;"><strong><span>&lt; 이 전</span></strong></a>
							<a href="#" class="b-btn type1" onclick="jsView('${dataVo.nextVO.domainCd}','${dataVo.nextVO.bbsCd}', '${dataVo.nextVO.bbscttSeq}', '${dataVo.nextVO.registPassword}', '${dataVo.nextVO.othbcAt}'); return false;"><strong><span>다 음 &gt;</span></strong></a>
						</c:if>
					</p>
					<p id="btn_modify_trash" class="fr">
