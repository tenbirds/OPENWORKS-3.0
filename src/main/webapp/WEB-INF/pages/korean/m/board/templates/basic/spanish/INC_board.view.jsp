...
<%@ taglib uri="http://www.openworks.kr/jsp/validate" prefix="valid"%>
    ...
    <!-- 내용보기 -->
        <table class="${boardConfVo.bbsCd eq 1001? 'tbl_view':'tbl_detail_view'}" >
            <caption>
                <strong>${dataVo.bbscttSj} Se provee información.</strong>
                <details>
                    <summary>${dataVo.bbscttSj} Se provee información.</summary>
                </details>
            </caption>
            <colgroup>
                            ...
                            <c:choose>
                                <c:when test="${boardConfVo.bbsCd eq '1009'}"> <!--  성공사례인 경우  -->
                                    ...
                                    <c:if test="${arrange.columnId eq 'RDCNT'}">
                                                <ul class="function">
                                                     <li>Cantidad de visualizaciones  <c:out value='${dataVo.rdcnt}'/></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    <c:if test="${arrange.columnId eq 'CTG_CD'}">
                                        ...
                                        <c:if test="${boardConfVo.ctgYn eq 'Y'}">
                                            <tr>
                                                ...
                                                <td class="tit">Fecha de publicacion</td>
                                ...
                                <c:otherwise>
                                    ...
                                    <c:if test="${arrange.columnId eq 'RDCNT'}">
                                               <span class="inquiry">Cantidad de visualizaciones  <c:out value='${dataVo.rdcnt}'/></span>
                                            </td>
                                    </c:if>
                                    <c:if test="${arrange.columnId eq 'REGIST_DT'}">
                                           <td>
                                                   ...
                                                   <ul class="function">
                                                       <c:if test="${boardConfVo.bbsCd ne '1009'and boardConfVo.bbsCd ne '1001' and boardConfVo.bbsCd ne '1002'}">
                                                           <c:if test="${!empty __usk.userKey && __usk.userKey eq dataVo.userKey}">
                                                               <li><a href="#"  onclick="jsUpdateForm('UPDATE');">Editar</a></li>
                                                               <li><a href="#" onclick="jsDelete();">Eliminar</a></li>
                                                           </c:if>
                                                       </c:if>
                                                   </ul>
                                               </div>
...
<!--                                                                 <span class="file">첨부파일이 없습니다.</span> -->
                        ...
                        <td colspan="2">
                            <div class="tag">
                                <c:if test="${!empty dataVo.bbsTags}">
                                    <img src="/resources/openworks/theme/default/images/icon/icon_tag.gif" alt="Etiqueta" class="vm" />
    ...
    <div class="btn_area">
        <div class="${boardConfVo.bbsCd eq '1001'? 'btn_r':'btn_l'}">
            <div class="btn_navi"><a href="#" onclick="jsList('<c:out value='${param.q_currPage}'/>');"><span class="list">Lista</span></a></div>
        </div>
        <div class="btn_r">
        <c:if test="${boardConfVo.bbsCd ne '1009'and boardConfVo.bbsCd ne '1001' and boardConfVo.bbsCd ne '1002'}">
            <c:if test="${!empty __usk.userKey && __usk.userKey eq dataVo.userKey}">
    ...
    <form name="dataForm" id="dataForm" method="post" action="BD_board.list.do">
        ...
        <c:if test="${boardConfVo.listViewCd == 1002}">
            <%@include file="/WEB-INF/pages/web/board/templates/common/spanish/INC_prev.next.list.jsp" %>
        </c:if>
        <!-- 다음/이전을 다음/이전으로 보기 끝 -->
