...
<div id="container">
    ...
    <div id="contents">
        <div class="visual">
            <img src="/resources/web/theme/english/images/bizmatching/img_bizmatching_visual.jpg" alt="Can't find the consulting you're looking for? You can more easily find the solutions or services you want through the Bizmatching service."/>
        </div>
        <!-- location -->
        <div class="location"><span class="home">Home</span><strong>Bizmatching</strong></div>
        <!-- //location -->
        <!-- 검색 폼 -->
            ...
            <section class="content rela">
                ...
                <div class="sech_form">
                    <div class="cont_wrap">
                        <div class="search">
                            ...
                            <input type="button" class="btn" id="sech" name="sech" title="search" onclick="jsSearch();"/>
                        </div>
                        <div class="btn_org">
                            <a href="#" onclick="jsSearchReset(); return false;" >View All</a>
                            <a href="#" onclick="jsInsertForm('INSERT'); return false; "><span class="request">Request</span></a>
                        </div>
                    </div>
                </div>
                ...
                <table class="tbl_list type_A">
                    ...
                    <thead>
                        <tr>
                            <!-- 번호(필수) -->
                            <th id="th-ROWNUM" scope="col"><span id="ROWNUM">No</span></th>
                            <!-- 목록 배치에서 설정한 항목 배치 -->
                            <c:forEach items="${boardConfVo.listArrange}" var="arrange" varStatus="status">
                                <c:choose>
                                    <c:when test="${arrange.columnId eq 'BBSCTT_SJ'}">
                                        <th id="th-${arrange.columnId}" scope="col"><span id="${arrange.columnId}">Inquiry title</span></th>
                                        <th id="th-BIZ_PROGRESSION" scope="col"><span id="BIZ_PROGRESSION">Condition</span></th>
                                    </c:when>
                                    <c:when test="${arrange.columnId eq 'ESTN_COLUMN6'}">
                                        <th id="th-${arrange.columnId}" scope="col"><span id="${arrange.columnId}">${arrange.langColumnNm}</span></th>
                                        <th id="th-COMMENT_CNT" scope="col"><span id="COMMENT_CNT">Number of matches</span></th>
                                    </c:when>
                                    <c:when test="${arrange.columnId eq 'REGIST_ID'}">
                                        <th id="th-${arrange.columnId}" scope="col"><span id="${arrange.columnId}">Registrant</span></th>
                                    </c:when>
                                    <c:otherwise>
                                        <th id="th-${arrange.columnId}" scope="col"><span  id="${arrange.columnId}">${arrange.langColumnNm}</span></th><!-- <c:if test="${status.last}"> class="lr_none"</c:if> -->
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
                        </tr>
                                ...
                                <c:forEach items="${boardConfVo.listArrange}" var="arrange" varStatus="arrStatus">
                                    <c:choose>
                                        <c:when test="${arrange.columnId eq 'BBSCTT_SJ'}">
                                            ...
                                            <!-- 상태 -->
                                            <td>
                                                <c:if test="${baseVo.bizProgression eq 'COM'}"><span class="condi_a">Complete</span></c:if>
                                                <c:if test="${baseVo.bizProgression eq 'INC'}"><span class="condi_b">Incomplete</span></c:if>
                                                <c:if test="${baseVo.bizProgression eq 'PRO'}"><span class="condi_d">In progress</span></c:if>
                                            </td>
                                        </c:when>
                                        <c:when test="${arrange.columnId eq 'ESTN_COLUMN6'}">
                                            <!-- 종료일 -->
