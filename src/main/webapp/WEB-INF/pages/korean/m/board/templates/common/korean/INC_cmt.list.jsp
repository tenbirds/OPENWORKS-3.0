										...
										<c:if test="${commentVo.userDeleteAt ne 'Y'}">
											<c:if test="${boardConfVo.bbsCd  ne 1010 }">
								                ...
								                <span id="btnModifyCancle${status.index+1}" style="display:none;">
									                <a href="#" class="s-btn type3" onclick="jsModifyFormRemove2('<c:out value='${status.index+1}'/>');" title="수정취소">수정취소</a>
								                </span>
								                <!-- 수정 버튼 -->
											    <span id="btnModify${status.index+1}">
											        <a href="#" class="s-btn type3" onclick="jsCommentModify('<c:out value='${commentVo.domainCd}'/>','<c:out value='${commentVo.bbsCd}'/>', '<c:out value='${commentVo.bbscttSeq}'/>', '<c:out value='${commentVo.cmtSeq}'/>', '<c:out value='${commentVo.registId}'/>','<c:out value='${status.index+1}'/>');" title="수정">수정</a>
											    </span>
											</c:if>
											<a href="#" class="s-btn type1" onclick="jsDeleteComment('<c:out value='${commentVo.domainCd}'/>','<c:out value='${commentVo.bbsCd}'/>', '<c:out value='${commentVo.bbscttSeq}'/>', '<c:out value='${commentVo.cmtSeq}'/>', '<c:out value='${commentVo.registId}'/>','<c:out value='${status.index+1}'/>');" title="삭제">삭제</a>
									    </c:if>
									</c:if>
								</div>
								<!-- //글 등록 정보 -->
