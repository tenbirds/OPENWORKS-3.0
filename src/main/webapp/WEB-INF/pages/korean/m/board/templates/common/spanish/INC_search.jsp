	...
	<div class="cont_wrap">
	    <div class="selected" >
		    <c:if test="${boardConfVo.bbsCd eq 1008}">
				...
				<select name="q_product" id="q_product" title="Título y contenido" style="width:150px;" onchange="jsListReq(1);">
		            <option value=""> Título y contenido </option>
	    ...
	    <div class="selected" >
			<select name="q_searchKeyType" id="q_searchKeyType" title="검색하실 단어 제목, 내용 중 선택" onchange="if($(this).val() != ''){$(this).next().focus().select();}" style="width:115px;">
				<option value=""> Seleccionar </option>
				<option value="BBSCTT_SJ_BBSCTT_CN" <c:if test="${param.q_searchKeyType eq 'BBSCTT_SJ_BBSCTT_CN'}"> selected="selected"</c:if>>Título y contenido</option>
			    
			    <c:if test="${boardConfVo.bbsCd ne 1001}"> <!-- 공지사항 사용안함 -->
				    <option value="REGISTER_NM___1002" <c:if test="${param.q_searchKeyType eq 'REGISTER_NM___1002'}"> selected="selected"</c:if>>Autor</option>
	            </c:if>			
	
			    <c:if test="${boardConfVo.bbsCd ne 1003 and boardConfVo.bbsCd ne 1002 and boardConfVo.bbsCd ne 1001}"> <!-- 묻고답하기, 자주찾는질문,공지사항 사용안함 -->
					<option value="NTT_CMT_CN" <c:if test="${param.q_searchKeyType eq 'NTT_CMT_CN'}"> selected="selected"</c:if>>Contenido de la respuesta</option>
					<option value="REGISTER_NM___1007" <c:if test="${param.q_searchKeyType eq 'REGISTER_NM___1007'}"> selected="selected"</c:if>>Respondido por</option>
				</c:if>
...
<!-- 		<input type="image" alt="검색옵션" src="/resources/web/theme/default/images/btn/btn_g_searchoption.gif" onclick="jsToggleSearchOption(this); return false;" /> -->
        ...
        <c:if test="${boardConfVo.bbsCd eq 1002}">
            <div class="btn_org"><a href="#" onclick="jsSearchReset(); return false;" >Ver todos</a></div>
        </c:if>
        <c:if test="${boardConfVo.bbsCd ne 1001 and boardConfVo.bbsCd ne 1009 and boardConfVo.bbsCd ne 1002 and boardConfVo.bbsCd ne 1003}">
            <div class="btn_org"><a href="#" onclick="jsInsertForm('INSERT');">Redactar</a></div>
<%--             <c:if test="${boardConfVo.bbsCd eq 1008}"> --%>
<!-- 	            <div class="btn_org"><a href="#" onclick="jsSearchReset(); return false;" />전체보기</a></div> -->
<%--             </c:if> --%>
        </c:if>
        <c:if test="${boardConfVo.bbsCd eq 1003}">
            <div class="btn_org"><a href="#" onclick="jsInsertForm('INSERT');">Enviar una pregunta</a></div>
        </c:if>
    </div>
</div>		
