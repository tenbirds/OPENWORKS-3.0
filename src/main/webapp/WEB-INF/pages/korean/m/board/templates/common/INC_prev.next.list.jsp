					...
					<c:choose>
						...
						<c:otherwise>
							<td class="t_al">
								...
								${fn:substring(dataVo.prevVO.bbscttSj,0,45)}<c:if test="${dataVo.prevVO.bbscttSjLength > 45}">...</c:if>
								</a>
								<!-- 비공개 글 여부 -->
								...
								<c:if test="${boardConfVo.cmtYn eq 'Y' && dataVo.prevVO.commentCnt > 0}">
								  <span class="num" title="의견글이 ${dataVo.prevVO.commentCnt}개 존재합니다.">[${dataVo.prevVO.commentCnt}]</span>
								</c:if>
								<!-- 새글 여부 -->
...
<!-- 									  <img src="/resources/web/theme/default/images/icon/icon_new.gif" alt="새글"/> -->
							...
							<c:choose>
							    ...
							    <c:otherwise>
									<td>${dataVo.prevVO.registerNm}</td>
							    </c:otherwise>
							</c:choose>
							...
							<td class="date">${dataVo.prevVO.registDt}</td>
						</c:otherwise>
					</c:choose>
				</c:if>
			</tr>
					...
					<c:choose>
						...
						<c:otherwise>
							<td class="t_al">
								...
								${fn:substring(dataVo.nextVO.bbscttSj,0,45)}<c:if test="${dataVo.nextVO.bbscttSjLength > 45}">...</c:if>
								</a>
								<!-- 비공개 글 여부 -->
...
<!-- 									  <img src="/resources/web/theme/default/images/icon/icon_new.gif" alt="새글"/> -->
							...
							<c:choose>
                                ...
                                <c:otherwise>
                                    <td>${dataVo.nextVO.registerNm}</td>
                                </c:otherwise>
                            </c:choose>
                            <c:if test="${boardConfVo.bbsCd eq '1003'}">
							...
							<td class="date">${dataVo.nextVO.registDt}</td>
						</c:otherwise>
					</c:choose>
				</c:if>
			</tr>
