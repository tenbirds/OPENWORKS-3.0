...
<%
    ...
    */
    ...
    String Param3Val = SubDomain+" "+"2";	//결과페이지에서 사용
    ...
    <script type="text/javascript">
            ...
            $.post("ND_findIdMailSend.do", {
                ...
                if(response) {
                    ...
                    if("${param.clTy}" == "2" || "${param.clTy}" == "3"){
                    	top.location.href = "/korean/pt/login/user/BD_findId3.do";
                    }else{
		...
		function goIDCheck(){
			if(bizrnoValidate() == true){
				...
				<% if("Y".equals(Config.getString("global.sslUse"))) { %>
					  document.inputForm.action = "https://<%=GlobalConfig.DOMAIN_SUBNM %>:446/common/bizrno/rnCheckRequest.do";
	            <% }else{ %>
...
<body>
	...
	<section class="pageTit">
		...
		<div class="conWrap">
			...
			<img src="/cloud_ver2/new_cloud/images/findid_step_2.png" alt="기본정보 입력" />
		</div>
		<!--// 타이틀 영역 -->
	</section>
	
	<div class="conWrap find_idpw idComp">
		<h2>아이디 일부확인</h2>
		<h3><i class="quot"><em>${param.resultId}</em> / 가입날짜: ${param.regDt}</i></h3>
		
		<div>
			<ul>
				<li>개인정보보호를 위해 아이디 끝자리는 ***로 표시됩니다. <br />전체 아이디를 찾으시려면 아이디 찾기에서 [전체아이디 이메일로 받기]를 이용 하시면 됩니다.</li>				
			</ul>
			
			<div class="btnArea rig">
				<a href="BD_index.do" class="btns st1 icn icnLogin">로그인</a>
				<a href="BD_findPwd.do" class="btns st1 icn icnPw">비밀번호 찾기</a>
			</div>
		</div>
	</div>
	<!--// content -->
			...
			<div class="contents">
				...
				<div class="section">
					...
					<div class="tab-type1">
						...
						<div class="step">
							...
							<span class="step03 on"><em>아이디 일부확인 및 인증정보 입력</em></span>
							<span class="step04"><em>전체아이디 확인</em></span>
						</div>
					</div>
					<!-- //tab-type1 -->
				</div>
				...
				<div class="section">
					<div class="type-list ac">
						<p class="txt-comform"><strong class="txt-point mr20">${param.resultId}</strong>가입날짜 : ${param.regDt}</p>
					</div>
				</div>
				<!-- //section -->
				<!-- btn -->
				<div class="btn-area ac">
					<a href="BD_index.do" class="b-btn type1"><strong><span>로그인</span></strong></a>
				...
				<!-- btn -->
				<div class="btn-area">
					<a href="#show" class="b-btn check type2"><strong><span class="down_gt">아이디 뒷자리 확인하기</span></strong></a>
				</div>
				<!-- //btn -->

				<div id="full_info" style="display:none" class="title-area mt30">
					<h2>아이디 뒷자리 확인을 위한 인증정보 입력</h2>

					<!-- [ 개인 구매회원 | 기업 구매회원 | 해외 회원 ]일  경우 인증방식 -->
					<span id="clty1" style="display:none">
					<div class="section">
						<div class="type-list ac">
							<p class="txt-comform">회원정보상의 <strong>이메일주소</strong>로 아이디를 보내드립니다.</p>
							<p class="txt-comform bor-t pt30">등록한 이메일<strong class="txt-point ml20">${param.maskEmail}</strong></p>
						</div>
					</div>
					<!-- btn -->
					<div class="btn-area ar">
						<a href="#none" onclick="jsFindSend();" class="b-btn auth type2"><strong><span>인증하기</span></strong></a>
					</div>
					<!-- //btn -->
					</span>
                    ...
                    <form name="form_ipin" method="post">
                        ...
                        <input type="hidden" name="param_r3" value="<%= Param3Val %>"/>	서브도메인+공백+요청페이지구분(2:아이디찾기)
                    </form>
					...
					<div class="join-area certification">
						<div class="multi-section-half">
							...
							<div class="ac">
								...
								<a href="#none" onclick="jsPopup(); return false;" title="아이핀 인증 팝업창 열림" class="b-btn auth type2"><strong><span>인증하기</span></strong></a>
								<div class="dot-list">
									<ul>
