...
<head>
    ...
    <title>컨설팅 신청 내역 | 쇼핑내역 | 마이페이지 | 씨앗 마켓</title>  
    <!--[if lt IE 9]>
...
////        document.dataForm.submit();
        $("#dataForm").attr("action", "BD_cnslList.do");
        $("#dataForm").submit();
    };
		...
		<div class="tab-search">
			<table class="tbl_list_input">
				...
				<tbody>
					<tr>
						<th>서비스</th>
						<td><input type="text" id="p_name" title="서비스명 입력"  class="w650" name="q_searchVal" value="<c:out value="${searchMap.q_searchVal}" />"></td>
					</tr>
					<tr>
						<th>기간</th>
						<td>
							<div class="calendar_input">
		                    <input type="text" class="input" id="q_beginDate" name="q_beginDate" readonly="readonly" title="검색 시작일 입력" style="width:100px;"
		                    <c:if test="${not empty searchMap.q_beginDate}"> value="${searchMap.q_beginDate}"</c:if> />
				            </div>
			                <span class="divide"> ~ </span>
			                <div class="calendar_input end">
			                <input type="text" class="input" id="q_endDate" name="q_endDate" readonly="readonly" title="검색 종료일 입력" style="width:100px;"
			...
			<table>
	            ...
	            <c:forEach items="${pager.list}" var="_bean" varStatus="status">
	                <tr>
	                    ...
	                    <td><fmt:formatNumber value="${_bean.goodsPc}" type="NUMBER" groupingUsed="true"/></td>
	                    <td>
	                        <span class="condi_a">${_bean.orderCnfirmAt}</span><br/>
	                        <span class="btn_light_gray02">
	                            <a href="javascript:;" title="팝업 창 열림" onclick="jsOrderView(this,'${baseVo.purchsNo}')">상세보기</a>
	                        </span>
	                    </td>
	                    <td>
