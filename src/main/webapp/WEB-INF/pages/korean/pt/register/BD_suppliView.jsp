...
<!-- //KICA SecuKit NX -->
	    ...
	    var jsSaveAction = function() {
	           if(checkEditor()) {	            
	            ...
	            if(confirm("보완신청 하시겠습니까?")) {
	                $("#dataForm").attr("action", "BD_suppliUpdateAction.do");
	                $("#dataForm").submit();
				}
			}
		};
	      ...
	      // 회사명
		   /*  if ($("#cmpnyNm").val()==null || $("#cmpnyNm").val().trim()=="") {
	        ...
	        /*  if($.trim($("#telno1").val()) != "" || $.trim($("#telno2").val()) != "" || $.trim($("#telno3").val()) != "" ){
	             if(!fnb_isNumeric($.trim($("#telno1").val()))) {
	                alert('대표번호를 입력해주세요.');
	                $("#telno1").focus();
	                return false;
	             }
 ...
 <body>
		...
		<!-- //KICA WebUI 영역 -->
			...
			<input type="hidden" name="bassAgremManageSeq" id="bassAgremManageSeq" value="${bassAgrem.bassAgremManageSeq}" />
			<input type="hidden" name="agremOrder" id="agremOrder" value="${bassAgrem.agremOrder}" />
			<input type="hidden" name="agremEndDe" id="agremEndDe" value="" />
			<input type="hidden" name="agremStat"  id="agremStat" value="1007"/>
		    <input type="hidden" name="bizrno"     id="bizrno"    value="${param.bizrno}">
		    <input type="hidden" name="screenType" id="screenType" value="${screenType}">

	<!-- content -->
			<div class="join_info inputOffer">
				<c:if test="${screenType eq 'myPage'}">
				<h4>협약상태 알림사항</h4>
				<table class="tbl_st1">
					<thead>
						<tr>
							<th style="padding: 12px 0px 12px 0px !important;">현 협약 차순</th>
							<th style="padding: 12px 0px 12px 0px !important;">권한 신청일</th>
							<th style="padding: 12px 0px 12px 0px !important;">승인일</th>
							<th style="padding: 12px 0px 12px 0px !important;">종료일</th>
							<th style="padding: 12px 0px 12px 0px !important;">상태</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td>${bassAgrem.agremOrder}차</td>
							<td>${bassAgrem.presentnDe}</td>
							<td>${bassAgrem.cnclsComptDe}</td>
							<td>${bassAgrem.agremEndDe}</td>
							<td>${bassAgrem.agremStat}</td>
						</tr>
					</tbody>
				</table>
				</c:if>				
				<c:if test="${not empty bassAgrem and bassAgrem.agremStatCd eq '1003'}" >
				<h4>한국정보화진흥원 알림사항</h4>
				<table class="tbl_st4">
					<tr>
						<th>상태</th>
					...
					<tr>
						<th>알림사항</th>
						<td class="importInfo_2"><c:if test="${ null eq bassAgrem.mangComt}">관리자 의견이 없습니다.</c:if><c:if test="${ null ne bassAgrem.mangComt}">${bassAgrem.mangComt}</c:if></td>
					</tr>
				</table>
				</c:if>
				...
				<table class="tbl_st4">
					<tr>
						...
						<td>
						<select id="cmpnyLoca" name="cmpnyLoca" style="width:100px;">
					...
					<tr>
						...
						<td colspan="3" class="thumb">							
							<span id="imgPrev1">
								...
								<img src="/cloud_ver2/new_cloud/images/temp/thumb_10.png" alt="대표이미지">
								<a href="#">변경</a> -->
							</span>
				...
				<table class="tbl_st4">
					<tr>
						...
						<td>
							<input type="hidden" name="bassAgrmntFileSeq" value="${bassAgrem.bassAgrmntFileSeq}" /> 
					...
					<tr>
						...
						<td>
							<input type="hidden" name="rgsbukTrnscrFileSeq" value="${bassAgrem.rgsbukTrnscrFileSeq}" /> 
							<c:forEach var="i" begin="1" end="1" step="1" varStatus="status">
								<div class="uploadFile"
								...
								<c:if test="${not empty bassAgrem.rgsbukTrnscrFile[i-1]}">
									<div class="uploaded">
										<span>
											...
											<a href="#none" onclick="jsFileDelete(this, ${bassAgrem.rgsbukTrnscrFile[i-1].fileSeq}, '${bassAgrem.rgsbukTrnscrFile[i-1].fileId}'); return false;">삭제</a>
										</span>
									</div>
								</c:if>
							</c:forEach>			            
					...
					<tr>
						...
						<td>
							<input type="hidden" name="cdltDocFileSeq" value="${bassAgrem.cdltDocFileSeq}" /> 
							<c:forEach var="i" begin="1" end="1" step="1" varStatus="status">
								<div class="uploadFile"
								...
								<c:if test="${not empty bassAgrem.cdltDocFile[i-1]}">
									<div class="uploaded">
										<span>
											...
											<a href="#none" onclick="jsFileDelete(this, ${bassAgrem.cdltDocFile[i-1].fileSeq}, '${bassAgrem.cdltDocFile[i-1].fileId}'); return false;">삭제</a>
										</span>
									</div>
								</c:if>
							</c:forEach>		            
			 	...
			 	<tr>
						...
						<td>
							<input type="hidden" name="fnlttFileSeq" value="${bassAgrem.fnlttFileSeq}" /> 
							<c:forEach var="i" begin="1" end="1" step="1" varStatus="status">
								<div class="uploadFile"
								...
								<c:if test="${not empty bassAgrem.fnlttFile[i-1]}">
									<div class="uploaded">
										<span>
											...
											<a href="#none" onclick="jsFileDelete(this, ${bassAgrem.fnlttFile[i-1].fileSeq}, '${bassAgrem.fnlttFile[i-1].fileId}'); return false;">삭제</a>
										</span>
									</div>
								</c:if>
							</c:forEach>		            
					...
					<tr>
						<th>벤처기업인증서</th>
						<td>
							<input type="hidden" name="vnentrCrtfFileSeq" value="${bassAgrem.vnentrCrtfFileSeq}" /> 
							<c:forEach var="i" begin="1" end="1" step="1" varStatus="status">
								<div class="uploadFile"
								...
								<c:if test="${not empty bassAgrem.vnentrCrtfFile[i-1]}">
									<div class="uploaded">
										<span>
											...
											<a href="#none" onclick="jsFileDelete(this, ${bassAgrem.vnentrCrtfFile[i-1].fileSeq}, '${bassAgrem.vnentrCrtfFile[i-1].fileId}'); return false;">삭제</a>
										</span>
									</div>
								</c:if>
							</c:forEach>		            
					...
					<tr>
						<th>직접생산증명서</th>
						<td>
							<input type="hidden" name="directPrdctnCrtfFileSeq" value="${bassAgrem.directPrdctnCrtfFileSeq}" /> 
							<c:forEach var="i" begin="1" end="1" step="1" varStatus="status">
								<div class="uploadFile"
								...
								<c:if test="${not empty bassAgrem.directPrdctnCrtfFile[i-1]}">
									<div class="uploaded">
										<span>
											...
											<a href="#none" onclick="jsFileDelete(this, ${bassAgrem.directPrdctnCrtfFile[i-1].fileSeq}, '${bassAgrem.directPrdctnCrtfFile[i-1].fileId}'); return false;">삭제</a>
										</span>
									</div>
								</c:if>
							</c:forEach>		            
				...
				<div class="btnArea rig">
					<a href="javascript:jsSaveAction();"  class="btns st1 icn icnCheck2">보완신청</a>
