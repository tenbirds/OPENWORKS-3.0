...
%>
		<div class="section">
          	<dl class="search-result">	
              	<dt>
              		<!-- IaaS 검색결과 -->
              		<h3>IaaS <i><%=resultColTotalCnt %></i>건</h3>
<%-- 					 이전 소스 : 화면에서 제외된 소스	
					...
					<input type="hidden" id="goodsTyCd" name="goodsTyCd" value="1003"/>
	              	<!-- 필터 적용 내용 출력 -->
		...
		<div>
			...
			<%
				for(int idx=0; idx < resultColCnt; idx++){
					...
					String GOODSMAINIMAGE = search_col.w3GetField("iaas_cloud", "GOODSMAINIMAGE", idx);
			%>
				...
				<tr>
					...
					<th rowspan="2" style="white-space:normal;">
						<a href="<%=URL%>" style="font-size:16px; color:#0186e1; word-break:keep-all;"><%=search_col.w3GetField("iaas_cloud", "GOODSNM", idx).replaceAll("<!HS>","<em class=\"txt-word\">").replaceAll("<!HE>","</em>") %></a>
						<img src="<%=GOODSMAINIMAGE%>" alt="서비스메인이미지" /></a>
					</th>
					<td style="white-space:normal;">
						<a href="#" onclick="jsSeller('<%=USERID%>');"><%=search_col.w3GetField("iaas_cloud", "LANGSTORENM", idx).replaceAll("<!HS>","<em class=\"txt-word\">").replaceAll("<!HE>","</em>") %></a>
						<a href="<%=URL%>"><%=search_col.w3GetField("iaas_cloud", "GOODSSUMRY", idx).replaceAll("<!HS>","<em class=\"txt-word\">").replaceAll("<!HE>","</em>") %></a>
					</td>
					<td>
		...
		<div class="btnArea rig">
			<a href="javascript:doCollection('iaas_cloud');"class="btns st1">+ 더보기</a>
		</div>
