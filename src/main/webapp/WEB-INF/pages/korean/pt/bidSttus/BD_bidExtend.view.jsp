...
<head>
    ...
    <script type="text/javascript">
	    ...
	    var returnList = function(){
	        document.listForm.action = "/korean/pt/bidSttus/BD_bidSttusList.do";
...
<body>
	<div class="conWrap">
	    <form name="listForm" id="listForm">
	        ...
	        <input type="hidden" name="searchKeyWord01" id="searchKeyWord01" value="${searchKeyWord01}"/>     <!--  목록 페이지 검색어1 -->
	        ...
	        <input type="hidden" name="searchKeyWord02" id="searchKeyWord02" value="${searchKeyWord02}"/>     <!--  목록 페이지 검색어2 -->
	        <input type="hidden" name="searchDiv03"     id="searchDiv03"     value="${searchDiv03}"/>         <!--  목록 페이지 조회조건3 -->	        
	    ...
	    <form name="dataForm" id="dataForm" method="post" enctype="multipart/form-data">
			...
			<div class="bidNoti bid">
				<h4>참가자격 변경등록 불이행에 따른 무효 안내사항</h4>
				<div>
					국가계약법 시행규칙 제44조 및 지방계약법 시행규칙 제42조, 입찰유의서에는 무효로 하는 입찰을 규정하고 있습니다.<br />
					특히, “참가자격 등록사항 중 상호, 대표자의 성명,을 변경등록하지 아니하고 공고참여서를 제출하여 무효되는 사례가 발생하고 있습니다.<br />
					참여자는 입찰참가자격 등록사항(씨앗 홈페이지의 제공자씨앗>제공자정보관리) 확인 및 공고 내용을 숙지하시어 불이익이 없도록하시기 바랍니다.
				</div>
	
				<h4>공고자 등록관련 공고무효 방지 안내사항</h4>
				<ul>
					<li>
					...
					<li>
						만일 귀하가 이미 퇴사 등으로 해당 업체의 공고대리인 자격을 상실한 경우<br />
						- 본 공고에는 법인등기부등본에 등재되어 있는 대표자나 다른 임직원, 입찰대리인이 투찰하여야 합니다.
					</li>
				</ul>
				...
				<table class="tbl_st4 e4">
					...
					<tr>
						...
						<td colspan="3" class="unit">
							...
							<!-- <p class="desc">※ 제안금액을 입력하시지 않으시면 해당 서비스의 가격표가 첨부 됩니다.</p> -->
						</td>
					</tr>
				</table>
	            ...
	            <c:choose>
	                <c:when test="${baseList.goodKndCd eq 1001}">
	                    <%@include file="goods/BD_Bid_Paas.form.jsp" %>
	                </c:when>
	                <c:when test="${baseList.goodKndCd eq 1002}">
	                    <%@include file="goods/BD_Bid_Saas.form.jsp" %>
	                </c:when>
	                <c:when test="${baseList.goodKndCd eq 1003}">
	                    <%@include file="goods/BD_Bid_Iaas.form.jsp" %>
	                </c:when>
	            </c:choose>
	            <!-- //tbl_write -->
