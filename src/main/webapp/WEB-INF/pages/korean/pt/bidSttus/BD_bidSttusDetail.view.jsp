...
<head>
    ...
    <script type="text/javascript">
        ...
        var returnList = function(){
            document.dataForm.action = "/korean/pt/bidSttus/BD_bidSttusList.do";    
		    document.dataForm.method = "GET";
		    document.dataForm.submit();
        }
        
        //  응찰 버튼
        var jsBidEntend = function(extendTermYn){
            // 응찰 마감일 뿐 아니라 입찰건이 마감되었는지 까지 체크
            if(extendTermYn != 'Y'){
                alert("입찰이 마감되었습니다.");
                return false;
            }
	        document.dataForm.action = "/korean/pt/bidSttus/BD_bidExtendView.do";
	...
	<div class="conWrap">
		...
		<div class="bidNoti">
			...
			<div class="btnArea rig">
			    <c:choose>
                    <c:when test="${baseList.bidKndCd eq '1001'}">
                        <c:if test="${baseList.bidPgesStat eq '1001' or baseList.bidPgesStat eq '1002'}">
                            <c:if test="${baseList.endsUserId ne sessionScope['__usk'].userId}">
                                <c:choose>
                                    <c:when test="${(baseList.cntrMothod eq '1004')}">                                    
						                <c:forEach items="${pager}" var="myWishList" varStatus="status">
						                    <c:if test="${myWishList.userId eq sessionScope['__usk'].userId}">
						                        <a href="#" onclick="jsBidEntend('${baseList.extendTermYn}'); return false;" class="btns st1 icn icnWrite">응찰</a>
						                    </c:if>
					                    </c:forEach>
                                    </c:when>
                                    <c:otherwise>
