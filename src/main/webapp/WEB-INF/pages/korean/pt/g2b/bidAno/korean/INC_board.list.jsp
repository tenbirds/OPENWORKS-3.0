...
<c:if test="${boardConfVo.bbsCd ne '1008' and boardConfVo.bbsCd  ne '1009' or boardConfVo.bbsCd eq '1011'}">
	...
	<table>
		<caption>
			...
			<details> <summary>기본게시판 목록으로 번호,분류,제목,작성자,작성일 등의 조회 정보를 제공합니다.</summary> </details>
		</caption>
		<colgroup>
					            ...
					            <c:if test="${arrange.columnId eq 'BBSCTT_SJ'}">
						            ...
						            <dl>						
							            <c:choose>
								            ...
								            <c:otherwise>
									            <dt><a href="#" rel="#titleSummary${status.count}" class="contentTip" onclick="jsView('<c:out value='${baseVo.domainCd}'/>','<c:out value='${baseVo.bbsCd}'/>', '<c:out value='${baseVo.bbscttSeq}'/>', '<c:out value='${baseVo.registPassword}'/>', '<c:out value='${baseVo.othbcAt}'/>'); return false;">										
