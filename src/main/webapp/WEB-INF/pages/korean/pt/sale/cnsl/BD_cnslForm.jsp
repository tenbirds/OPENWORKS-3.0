...
//     jsNationChange('00');
    if ($("input[name=langCode]:checked").val() == '00') {
        $("#nation").hide();
    } else {
        $("#nation").show();
    }
...
var jsGoodsInsertAction = function(sttus) {
...
//             $("input[type=file][value!='']").trigger("onchange");
    ...
    $("#tempSave").attr("onclick", "if (jsLoading()==true){jsGoodsInsertAction('1001');};");
    $("#regReq").attr("onclick", "if (jsLoading()==true){jsGoodsInsertAction('1002');};");
};
...
var checkEditor2 = function() {
    ...
    if(eachSection == 'N'){
        ...
        if($.trim($("#goodsChargerCttpc1").val()) != "" || $.trim($("#goodsChargerCttpc2").val()) != "" || $.trim($("#goodsChargerCttpc3").val()) != "" ){
            ...
            if(langCode != '00' && $("#goodsChargerCttpc0 option:selected").val() == "") {
                alert('국가를 선택해주세요.');
...
var checkEditor = function() {
 ...
 var count = 0;
    ...
    if(eachSection == 'N'){
       //지역
        if ($("#metaAREAs").val()==null || $("#metaAREAs").val()=="") {
           alert("지역 항목을 입력하여 주세요.");
           document.getElementById("metaAREAsBtn").focus();
           return false;
        }
...
//               alert(nm+'바이트까지 입력이 가능합니다.');
 ...
 var jsMetaPop = function(el, codeGroupCd, cdStart, cdEnd) {
     var codeLangCode = $("input[name=langCode]:checked").val();
     $(el).colorbox({
         title : "선택",
...
<input type="hidden"  value="" id="langCode"/>
				                            ...
				                            <li id ="goodsCtgry">
				                                <c:forEach items="${dataVo.goodsCtgry}" var="_goodsCtgry" varStatus="status">
				                                    <ul class="multi-list langCategory${category.CATEGORY_LANGCD} goodsCtgry_${category.CTGRYCODE}">
				                                        <li class="cate01"><span>· <c:out value='${_goodsCtgry.CTGRYCODE}'/></span></li>
				                                        <li class="cate02"><span><c:out value='${_goodsCtgry.CTGRYCLNM }'/></span> <span><c:out value='${_goodsCtgry.CTGRYPATH}'/></span>
												...
												<ul class="multi-section file-upload mt20">
								                   <c:if test="${empty dataVo.imageFile}">
					                                 <c:forEach var="i" begin="1" end="10" step="1" varStatus="status">
					                                    <li id="imgPrev${status.index}">
					                                    	<div>
					                                    	<p class="thumb"></p>
					                                    	<p class="ac">
					                                        <a href="#" class="s-btn type5"><strong><span>등록</span></strong></a>
					                                        </p>
								                                <a href="#" onclick="jsFileRemove('${status.index}'); return false;"><img src="/new_cloud/images/icon-btn/btn-close.png" alt="" /></a>
...
<%-- 2016.12.09 수정  소계영상, 홈페이지URL 삭제요청으로 주석처리함								
								...
								<tr>
									...
									<td>
									 <span class="agree">
									 <c:forEach items="${goodsMetaPrice}" var="_goodsMetaPrice" varStatus="status">
			                              <input name="metaPrices" id="metaPrice${_goodsMetaPrice.metaPriceCd}" type="checkbox" value="${_goodsMetaPrice.metaPriceCd}"
			                                    <c:if test="${(not status.first) and (not status.last)}"> </c:if>
			                                    <c:if test="${status.first}"> checked="checked"</c:if>/>
			                               		<span class="caption mr10">${_goodsMetaPrice.metaPriceNm}</span>
			                          </c:forEach>
								...
								<tr>
									...
									<td><a id="metaAREAsBtn" onclick="jsMetaPop(this, 2022, '', '');" href="javascript:;"  class="b-btn type1"><strong><span>선택</span></strong></a>
										<span id="metaAreanm"></span>
		                            	<input name="metaAreas" id="metaAreas" type="hidden" value=""/>
									</td>
								</tr>
								<tr>
									<th><span class="point">A/S방침</span></th>
									<td><a id="metaASsBtn" onclick="jsMetaPop(this, 1021, '1000', '1999');" href="javascript:;"  class="b-btn type1"><strong><span>선택</span></strong></a>
								...
								<tr>
									...
									<td><input type="text" id="spcltyRealm" name="spcltyRealm" class="w650" /></td>
								</tr>
								<tr>
									<th><span class="point">전문<br/>컨설턴트 수</span></th>
									<td><input type="text" id="cnstntCo" name="cnstntCo" class="w650" /></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td><input type="text" id="langStoreNm" value="<c:out value='${goodsStoreList.LANGSTORENM }'/>" name="langStoreNm" readonly="readonly"class="w650" /></td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td><input type="text" id="goodsChargerEmail1" name="goodsChargerEmail1" value="${fn:split(goodsStoreList.REPRSNTEMAIL,'@')[0]}" class="w194" /><span class="space">@</span><input type="text" name="goodsChargerEmail2" value="${fn:split(goodsStoreList.REPRSNTEMAIL,'@')[1]}" id="goodsChargerEmail2" class="w194" /></td>
								</tr>
								<tr>
			                        ...
			                        <td>
													...
													<div class="selected">
			                                            <select id="goodsChargerCttpc0" name="goodsChargerCttpc0" title="국가 선택" style="width:210px;">
			                                                ...
			                                                <c:forEach items="${nation}" var="list">
			                                                    <option value="<c:out value='${list.INTRLTELNL}'/>" ${sessionScope['__usk'].nationCode == list.NATIONCODE ? 'selected="selected"' : ''}><c:out value="${list.LANGNATIONNM}" />(<c:out value="${list.INTRLTELNL}" />)</option>
			                                                </c:forEach>
			                                            </select>
													</div>														
												</div>
										...
										<!-- //static select -->
			                            <input type="text" name="goodsChargerCttpc1" id="goodsChargerCttpc1" title="연락처 입력" value="${fn:split(goodsStoreList.REPRSNTTELNO,'-')[0]}" class="w124" maxlength="4"/> -
			                            <input type="text" name="goodsChargerCttpc2" id="goodsChargerCttpc2" title="연락처 입력" value="${fn:split(goodsStoreList.REPRSNTTELNO,'-')[1]}" class="w124" maxlength="4"/> -
			                            <input type="text" name="goodsChargerCttpc3" id="goodsChargerCttpc3" title="연락처 입력" value="${fn:split(goodsStoreList.REPRSNTTELNO,'-')[2]}" class="w124" maxlength="4"/>
			                        </td>
			                    </tr>
			                    <tr>
			                        <th><span class="point">담당자 휴대폰</span></th>
			                        <td>
			                            <input type="text" name="goodsChargerMbtlnum1" id="goodsChargerMbtlnum1" title="휴대전화번호 입력" value="" class="w124" maxlength="3"/> -
			                            <input type="text" name="goodsChargerMbtlnum2" id="goodsChargerMbtlnum2" title="휴대전화번호 입력" value="" class="w124" maxlength="4"/> -
			                            <input type="text" name="goodsChargerMbtlnum3" id="goodsChargerMbtlnum3" title="휴대전화번호 입력" value="" class="w124" maxlength="4"/>
			                        </td>
			                    </tr>
<!-- 	2016.12.09 수정  소계영상, 홈페이지URL 삭제요청으로 주석처리함		                    
								<tr>
									<th><span>홈페이지 URL</span></th>
									<td><input type="text" id="goodsHmpgUrl" name="goodsHmpgUrl" value="" class="w650" id="" /></td>
								</tr> -->
							</tbody>
						</table>
					</div>
							...
							<tbody>
								<tr>
									<th>
										...
										<div class="mt5">
											<textarea name="goodsSumry" id="goodsSumry" onkeyup="javascript:displayBytes(3000,'goodsSumry');" class="h167"></textarea>
											<p class="ar mt5">
								...
								<tr>
									<th>
										...
										<div class="mt5 border-box">
											...
											<ul id="metaTAGhtml">
				                                <!--<li><a href="#">윈도우 X</a></li>-->
				                                <!--<li><a href="#">인증서보유 X</a></li>-->
				                                <!--<li><a href="#">인증서보유 X</a></li>-->
				                                <!--<li><a href="#">표준프레임워크 X</a></li>-->
				                                <!--<li><a href="#">윈도우 X</a></li>-->
				                            </ul>
								...
								<tr>
									<th>
										...
										<div class="mt5">
											 <textarea name="cnslExpcEffect" id="cnslExpcEffect" rows="15" style="width: 743px" title="내용을 입력해주세요"></textarea>
					                        <script type="text/javascript">
					                        //<![CDATA[
										...
										<li class="mt10">
											...
											<div class="fr">
												...
												<div class="file-area fr">
													...
													<span class="file_del"><img onclick="jsInputFileReset('dnlFile1','dnfileInput1');" src="/new_cloud/images/icon-btn/btn-close.png" alt="삭제"></span>
													<div class="fake_input">
														<a href="#" class="b-btn type1"><strong><span>파일선택</span></strong></a>
														<span id="dnlFile1Span">
				...
				<div class="btn-area mt20">
					...
					<p class="fr">
						<a href="javascript:" id="tempSave" onclick="if (jsLoading()==true){jsGoodsInsertAction('1001');};" class="b-btn save2 mr10 type1"><strong><span>임시저장</span></strong></a>
						<a href="javascript:" id="regReq" onclick="if (jsLoading()==true){jsGoodsInsertAction('1002');};" class="b-btn registry type2"><strong><span>등록신청</span></strong></a>
					</p>
				</div>
				</form>
			</div>
